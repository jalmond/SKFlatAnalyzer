...
if IndividualSample:
    ...
    if SampleExists(args):
        ...
        RunCommand(TestMode,f"SKFlat.py -a {analyzer} -i {args.samplename} -n {njob} --nmax {nmax} -e {args.era} --skim SkimTree_HNMultiLepBDT {FlagCommand(SampleType, flags)}&")
    exit()


if args.WZ:
    RunCommand(TestMode,f"SKFlat.py -a {analyzer} -i WZTo3LNu_mllmin4p0_powheg -n 200 --nmax {nmax} -e 2018 --skim SkimTree_HNMultiLepBDT {FlagCommand('RunPrompt', flags)}&")
    exit()
...
if args.Central or args.Systematics:
    for era in era_list:
        ...
        if RunConv:
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Conv/ConvWG.txt        -n 75        --nmax {nmax}   -e {era} --skim SkimTree_DileptonBDT  {FlagCommand('RunConv', flags)}  &")
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Conv/Conv.txt          -n 75        --nmax {nmax}   -e {era} --skim SkimTree_HNMultiLepBDT  {FlagCommand('RunConv', flags)}  &")
        ...
        if RunCF:
            ### CF                                                                                                                                                                               
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EMu.txt      -n 100    --nmax {nmax}   -e {era} --skim SkimTree_HNMultiLepBDT  {FlagCommand('RunCF', flags)} ")
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EE.txt      -n 100    --nmax {nmax}   -e {era} --skim SkimTree_HNMultiLepBDT  {FlagCommand('RunCF', flags)} ")
