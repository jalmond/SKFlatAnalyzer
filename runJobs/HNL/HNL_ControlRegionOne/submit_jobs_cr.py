...
if IndividualSample:
    ...
    if SampleExists(args):
        ...
        RunCommand(TestMode,f"SKFlat.py -a {analyzer} -i {args.samplename} -n {njob} --nmax {nmax} -e {args.era} --skim SkimTree_AK8BDT {FlagCommand(SampleType, flags)}&")
    exit()


if args.WZ:
    RunCommand(TestMode,f"SKFlat.py -a {analyzer} -i WZTo3LNu_mllmin4p0_powheg -n 200 --nmax {nmax} -e 2018 --skim SkimTree_AK8BDT {FlagCommand('RunPrompt', flags)}&")
    exit()
...
if args.Central or args.Systematics:
    for era in era_list:
        ...
        if RunData:
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_MuMu.txt     -n 100  --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('', flags)} &")   
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EMu.txt      -n 100  --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('', flags)} &")   
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EE.txt       -n 100  --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('', flags)} &")   

        #### Prompt            
        if RunPrompt:
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Prompt/PromptSS.txt             -n 20        --nmax {nmax}   -e {era}  --skim SkimTree_AK8BDT   {FlagCommand('RunPrompt', flags)}  &")   
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Prompt/PromptSS2.txt            -n 200       --nmax {nmax}   -e {era}  --skim SkimTree_AK8BDT   {FlagCommand('RunPrompt', flags)}  &")   
            if args.RunAlternativePrompt or user == "jalmond":
                RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Prompt/PromptSSAlt.txt          -n 200       --nmax {nmax}   -e {era}  --skim SkimTree_AK8BDT   {FlagCommand('RunPrompt', flags)}  ")   

        ### Conv                                                                                                                                                                             

        if RunConv:
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Conv/ConvWG.txt        -n 50        --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('RunConv', flags)}  &")
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {mcpath}/Conv/Conv.txt          -n 50        --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('RunConv', flags)}  &")

        if RunFake:
            ### Fakes                                                                                                                                                                            
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EMu.txt     -n 100  --nmax {nmax}   -e {era}  --skim SkimTree_AK8BDT   {FlagCommand('RunFake', flags)} &")
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_MuMu.txt    -n 100  --nmax {nmax}   -e {era}  --skim SkimTree_AK8BDT  {FlagCommand('RunFake', flags)} &")
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EE.txt      -n 100    --nmax {nmax}   -e {era}  --skim SkimTree_AK8BDT  {FlagCommand('RunFake', flags)} ")
            
        if RunCF:
            ### CF                                                                                                                                                                               
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EMu.txt      -n 100    --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('RunCF', flags)} ")
            RunCommand(TestMode,f"SKFlat.py -a {analyzer}  -l {datapath}/{DATADir}/{era}_DiLepton_EE.txt      -n 100    --nmax {nmax}   -e {era} --skim SkimTree_AK8BDT  {FlagCommand('RunCF', flags)} ")
