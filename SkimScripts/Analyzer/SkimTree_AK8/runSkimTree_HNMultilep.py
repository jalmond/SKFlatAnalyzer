...
def run_data():
    ...
    era_list = ["2016postVFP", "2016preVFP", "2017"]
    for era in era_list:
        for dataset, jobs, skimcmd in [
                (f"DATA_EE_{era}.txt", njobs_data," --skim SkimTree_Dilepton " ),
                (f"DATA_EMu_{era}.txt", njobs_data," "),
        ]:
...
def run_all():
    ...
    for era in era_list:
        for dataset, jobs in [
            ...
            cmd = f"SKFlat.py -a {analyzer} -l {datapath}/{dataset} -n {jobs} --nmax {nmax} -e {era} &"
            run_command(cmd)
