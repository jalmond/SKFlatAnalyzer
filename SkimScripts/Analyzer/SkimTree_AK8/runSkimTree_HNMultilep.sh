...
run_new_samples() {
    ...
    for era in "${era_list[@]}"; do
        ...
        SKFlat.py -a "${analyzer}" -i "GluGluToZZto4tau" -n "${njobs}" --nmax "${nmax}" -e "${era}" &
	SKFlat.py -a "${analyzer}" -i "GluGluToZZto2mu2tau" -n "${njobs}" --nmax "${nmax}" -e "${era}" &
	SKFlat.py -a "${analyzer}" -i "GluGluToZZto2e2tau" -n "${njobs}" --nmax "${nmax}" -e "${era}" &
    done
