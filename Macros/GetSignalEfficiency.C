...
void GetSignalEfficiency(){ 
  ...
  TString outfile = "root/HNEfficiencies_highmass.root";
  ...
  for(unsigned int j = 0 ; j < SR.size(); ++j){
	...
	if ( channel[k].first.Contains("Mup") ){
	  if(_sr == "SR2")  sign= "_nsigsame_sign";    
	  if( _sign ==  "SignalOS__" && _sr == "SR2") sign= "_nsigopposite_sign";
	}
	...
	for(unsigned int i = 0 ; i < masses.size(); ++i){
	  ...
	  cout << "/data6/Users/jalmond/SKFlatOutput//Run2Legacy_v4/HNtypeI_JA/2016/"+_sign+"/HNtypeI_JA_HN_Schannel_"+channel[k].first+"_"+im+"_nlo.root" <<endl;
	  ...
	  if(filemm->IsZombie()) {delete filemm; continue;}
	  if(filepp->IsZombie()) { delete filepp;  continue;}
	  if(!filemm) continue;
	  if(!filepp) continue;
	  ...
	  cout << "CutFlow/NoCut_HNtypeI_JA_"+channel_lab + "_"+ ID << endl;
