...
def CreatDATAFile(alias_list_,y,skim, analyzer):
    dirpath="/data6/Users/jalmond/SKFlatOutput//Run2Legacy_v4/"+analyzer+"/"+y+"/DATA/"

    jobs_run=False
    listpath_all="/data6/Users/jalmond/2020/HL_SKFlatAnalyzer/SKFlatAnalyzer/run/lists/DATA_"+y + "_"+skim+".txt"
    if skim == "":
        listpath_all="/data6/Users/jalmond/2020/HL_SKFlatAnalyzer/SKFlatAnalyzer/run/lists/DATA_"+y + ".txt"

    listpath="DATACheckSkim_"+y + "_"+skim+".txt"
    list_ = open (listpath,"w")
...
for y in years:
    ...
    for f in listdir("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/"):
        ...
        if VERBOSE:
            ...
            os.system("ls /gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/" + f  )

        for d in listdir("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/"+f):
                        
            for d2 in listdir("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/"+f + "/"+d):
                nFiles=0
                for files in listdir("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/"+f + "/"+d+"/"+d2 ):
                    if ".root" in files:
                        nFiles=nFiles+1
                if VERBOSE:
                    print "NFiles = " + str(nFiles)
                    _samples.append("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/"+f  + "/"+d)
                    _dir.append("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/" )
                    _alias.append("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/" +f+":"+d.replace('period',''))
                    _date_tags.append("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/" +f  +d2)
                    _nFiles.append("/gv0/DATA/SKFlat/"+tag+"/"+y+"/DATA"+skim_tmp+"/"+f +str(nFiles))
                    _year.append(y)
...
while count < len(_samples):
    ...
    if not _poss_runBATCH:
        if not _ranbatch:
            ...
            for _y in years:
                path_mc=CreatDATAFile(alias_list_run,_y,"","CheckSkim" )
                path_mc_skim=CreatDATAFile(alias_list_run,_y,skim,"CheckSkim" )
