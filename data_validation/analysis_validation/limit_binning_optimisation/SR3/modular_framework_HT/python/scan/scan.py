...
def run_scan_group(data, flavs, masses, config):
    ...
    for met in tqdm(config["mets"], desc="MET", leave=False):
        ...
        for cat in data[met]:
            ...
            # ----------------------------------------
            # BUILD ARRAYS (possibly per era)
            # ----------------------------------------
            tqdm.write (f" MET = {met} CAT = {cat} [build_arrays_multi]")
                        
            arrays, bin_lo = build_arrays_multi(data, met, cat, flavs, masses)
            
            internal_edges = edges_full[1:-1]
            
            # expected combinations (before min_width cut)
            ...
            tqdm.write(
                ...
                f"[SCAN] FLAV={flav_label} | MASS={mass_label} "
                
                f"| MET={met} | CAT={cat} "
                
                f"-> {len(binnings)} combinations (nbins={nbins_test})"
        ...
        per_met_results.append({
            "met": met,
            "Z": Z_met,
            "regions": regions_out  
        })

        if Z_met > best_global["Z"]:
            best_global["Z"] = Z_met
            best_global["met"] = met
            best_global["regions"] = regions_out
