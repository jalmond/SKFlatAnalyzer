...
def arrays_rebin_by_factor(edges, bkg, err2, sig, factor):
    """
    Merge every 'factor' consecutive bins into one (uniform or not).
    Keeps physical edges; sums contents and variances.
    """
    if factor <= 1:
        return edges, bkg, err2, sig
    M = len(bkg)
    ...
    ne2 = []
    ns = []
    for start in range(0, M, factor):
        stop = min(start + factor, M)
        nb.append(np.sum(bkg[start:stop]))
        ne2.append(np.sum(err2[start:stop]))
        ns.append(np.sum(sig[start:stop]))
        new_edges.append(edges[stop])
    return np.asarray(new_edges, float), np.asarray(nb, float), np.asarray(ne2, float), np.asarray(ns, float)


def snap_constraint_to_edges(x_last_min, edges):
    """
    Given a constraint x_last_min in the original binning, snap it
    to the next available (>=) lower edge in the (possibly) coarsened 'edges'.
    """
    idx = int(np.searchsorted(edges, x_last_min, side='left'))
...
# -------------------------
# Cross-mass evaluation (per-scan; returns dict)
# -------------------------
...
def evaluate_binning_for_masses(selected_dir, era, flavour, base_path, hpath,
                                ...
                                rebin_factor_for_sig=1):
    """
    Returns: dict like { "400": total_fom_float, ... }
    """
    ...
    for m in masses:
        ...
        edges_raw, _, _, sig_arr_raw = extract_edges_and_arrays(bkg_hist_for_shape, sig_hist)

        # coarsen the signal by the same factor used for DP arrays (if any)
        if rebin_factor_for_sig > 1:
            _, _, _, sig_arr = arrays_rebin_by_factor(edges_raw,
                                                      np.zeros_like(sig_arr_raw),
                                                      np.zeros_like(sig_arr_raw),
                                                      sig_arr_raw,
                                                      rebin_factor_for_sig)
            sig_arr = sig_arr[:len(bkg)]
        else:
            sig_arr = sig_arr_raw[:len(bkg)]
        ...
        per_mass_totals[m] = total_fom_m
        f_sig_tmp.Close()

        # Per-scan print (kept for visibility; combined is printed later)
        log_print(f"[EVAL] Mass {m}: Total FOM (this scan) = {total_fom_m:.2f}")

    return per_mass_totals
...
def run_scan(label, base_path, hpath, args, selected_dir, log_dir="log_scan"):
    ...
    hpath: 'MainPlots/L_T' (LowJet) or 'Mass/DiJet_M_l1jj' (DiJet)
    """
    ...
    with open(log_file, 'w') as log:
        ...
        # Existing FOM (limit binning; for info only)
        ...
        if (not sig_hist) or (not bkg_hist):
            ...
            return {
                ...
                "mass_totals": {}, "rebin_factor": 1,
            }

        # Right-merge constraints (from original bkg histogram)
        bins_info = get_bin_info(bkg_hist)
        _, _, x_last_min_raw, x_upper_raw = strict_merge_from_right(
            bins_info, min_bkg=args.min_bkg, max_rel_err=args.max_rel_err
        )

        # Arrays and prefix sums (possibly re-binned for LowJet)
        edges, bkg_arr, err2_arr, sig_arr = extract_edges_and_arrays(bkg_hist, sig_hist)

        # If LowJet/L_T, rebin by fixed factor (default 10)
        if label.lower() == "lowjet":
            rebin_factor_used = max(1, args.rebin_lowjet)
        else:
            rebin_factor_used = 1

        if rebin_factor_used > 1:
            edges, bkg_arr, err2_arr, sig_arr = arrays_rebin_by_factor(edges, bkg_arr, err2_arr, sig_arr, rebin_factor_used)
            # snap the last-bin lower-edge constraint onto the coarsened grid
            x_last_min = snap_constraint_to_edges(x_last_min_raw, edges)
            x_upper = edges[-1]
            log_print(f"[INFO] Re-binned {label} by factor={rebin_factor_used}: now {len(bkg_arr)} bins")
        else:
            x_last_min = x_last_min_raw
            x_upper = x_upper_raw
    ...
    return {
        ...
        "base_path": base_path,
        "hpath": hpath,
        "mass_totals": per_mass_totals,   # dict[str->float]
        "rebin_factor": rebin_factor_used,
    }
...
def main():
    ...
    # Consistent signal flavor
    parser.add_argument('--sig-tag', default='DYVBF_',
                        help="Tag inserted after 'HNL_' in the signal filename, e.g. '' or 'DYVBF_'")

    # LowJet fixed rebin factor (10 as requested)
    parser.add_argument('--rebin-lowjet', type=int, default=10,
                        help="Rebin factor for LowJet L_T histogram (e.g., 10 merges 10 original bins)")
    ...
    # --- Scan 1: LowJet with MainPlots/L_T (rebinned by factor) ---
    res_lowjet = run_scan(
        label="LowJet",
    ...
    # --- Scan 2: DiJet with Mass/DiJet_M_l1jj (as before) ---
    res_dijet = run_scan(
        label="DiJet",
    ...
    # header part (as before)
    lines = []
    lines.append("========== DP Scan Summary ==========")
    lines.append(f"Era: {args.era}  Flavour: {args.flavour}  Mass(anchor): {args.mass}  SigTag: {args.sig_tag}")
    lines.append("")
    ...
    # ---- Combined per-mass table: (LowJet + DiJet) vs Existing SR3 ----
    masses = ["400", "500", "600", "700", "800", "900", "1000"]

    # Open Bkg once for Existing calc
    bkg_file_path = os.path.join(selected_dir, args.era, "HNL_SignalRegion_Plotter_Bkg.root")
    f_bkg_all = ROOT.TFile(bkg_file_path)

    lines.append("Per-mass Combined Evaluation (LowJet + DiJet vs Existing SR3):")
    lines.append("Mass | LowJet | DiJet | Combined | Existing | Ratio")
    lines.append("-----|--------|-------|----------|----------|------")

    for m in masses:
        # existing baseline for SR3 (same as earlier logic)
        sig_path_m = os.path.join(selected_dir, args.era, f"HNL_SignalRegion_Plotter_HNL_{args.sig_tag}{m}.root")
        f_sig_m = ROOT.TFile(sig_path_m)
        existing_all = compute_existing_total_fom_allbins(f_sig_m, f_bkg_all, args.flavour, m)

        lj = res_lowjet["mass_totals"].get(m, 0.0)
        dj = res_dijet["mass_totals"].get(m, 0.0)
        comb = lj + dj
        ratio = (comb / existing_all) if existing_all > 0 else float('inf')
        lines.append(f"{m} | {lj:.2f} | {dj:.2f} | {comb:.2f} | {existing_all:.2f} | {ratio:.2f}")
        print(f"[COMBINED] Mass {m}: LowJet={lj:.2f}, DiJet={dj:.2f}, Combined={comb:.2f}, Existing={existing_all:.2f}, Ratio={ratio:.2f}")
