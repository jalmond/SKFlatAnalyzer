...
def main():
    ...
    for nb in NBINS_TO_SCAN:
        ...
        print (f"print_final_summary low 450-2000")
        print_final_summary(res_dp_raw_global_sig_w,"All Masses")
        

        res_dp_raw_global_sig_low_w = evaluate_dp_per_flavour_global_mass_run2_weighted_refbins(data, nb,use_fake_corr=USE_FAKE_FIX,run_z_no_unc=RUN_Z_NO_UNC,mass_list = {"400","450","500","600"})
        res_dp_raw_global_sig_med_w = evaluate_dp_per_flavour_global_mass_run2_weighted_refbins(data, nb,use_fake_corr=USE_FAKE_FIX,run_z_no_unc=RUN_Z_NO_UNC,mass_list = {"700","800","900"})
        res_dp_raw_global_sig_high_w = evaluate_dp_per_flavour_global_mass_run2_weighted_refbins(data, nb,use_fake_corr=USE_FAKE_FIX,run_z_no_unc=RUN_Z_NO_UNC,mass_list = {"1000","1100","1200","1300","1500","2000","2500"})
        print (f"print_final_summary low 450-600")
        print_final_summary(res_dp_raw_global_sig_low_w,"450-600")
        print (f"print_final_summary med 700-900")
        print_final_summary(res_dp_raw_global_sig_med_w,"700-900")
        print (f"print_final_summary high 1000+")
        print_final_summary(res_dp_raw_global_sig_high_w,"1000+")
        ...
        if nb == NBINS_TO_SCAN[-1]:
            ...
            "results": convert_results_for_plot(res_dp_raw_global_sig_w, mode="run2"),
                ...
                "label": f"Scan (GlobSig[weighted], All Masses Run2, N_bins={nb})"
            })

            scan_results_for_plots.append({
                "results": convert_results_for_plot(res_dp_raw_global_sig_low_w, mode="run2"),
                "raw": res_dp_raw_global_sig_low_w,
                "label": f"Scan (GlobSig[weighted], Low Run2, N_bins={nb})"
            })
            scan_results_for_plots.append({
                "results": convert_results_for_plot(res_dp_raw_global_sig_med_w, mode="run2"),
                "raw": res_dp_raw_global_sig_med_w,
                "label": f"Scan (GlobSig[weighted], Med Run2, N_bins={nb})"
            })

            scan_results_for_plots.append({
                "results": convert_results_for_plot(res_dp_raw_global_sig_high_w, mode="run2"),
                "raw": res_dp_raw_global_sig_high_w,
                "label": f"Scan (GlobSig[weighted], High Run2, N_bins={nb})"
            })
