...
def main():
    ...
    fixed_bins_mumu = [
        0.0, 200,300,  365.0, 395.0, 440.0, 485.0,580.0, 625.0, 680, 770.0, 850.0, 1000.0, 1185,5000.0
    ]
    fixed_bins_ee = [
        0.0, 390.0, 440.0, 490.0, 560.0, 600.0, 655.0, 685.0, 755.0, 785.0, 870.0,  1075.0, 1185.0, 1420.0, 5000.0
    ]
    fixed_bins_emu = [
        0.0, 385.0, 445.0, 490.0, 525, 565.0, 595.0, 640.0, 680.0, 760.0, 830, 890.0, 1080.0, 1450.0, 5000.0
    ]

    for flav in FLAVOURS:
        if flav == "MuMu":
            fixed_bins = fixed_bins_mumu
        if flav == "EE":
            fixed_bins = fixed_bins_ee
        if flav == "EMu":
            fixed_bins = fixed_bins_emu

        for mass in MASSES:
            valid = validate_binning_with_stat(
            ...
            print(f"[STAT CHECK] {flav:4s} mass={mass:5s} -> {status}")
                
    compare_fixed_vs_permass(
        data,
        fixed_bins,
        sr1bins_mm_byMass,
        sr1bins_ee_byMass,
