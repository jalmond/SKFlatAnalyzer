...
def evaluate_edges(edges, bin_cache, scan_era="Run2", debug=False):
    ...
    for i in range(len(edges) - 1):
        ...
        for era, (centers, cumsum_b, cumsum_f, cumsum_s, edge_indices) in bin_cache.items():
            ...
            if debug:
                per_era_info.append((era, sig, bkg, fake))

            if bkg < 0.15:
                valid_bin = False
                if debug:
                    print(f"[FAIL BKG] {era} [{x_low:.3f},{x_high:.3f}] B={bkg:.4f}")
                break
        ...
        if run2_bkg >= 1:
            ...
            fom = calculate_fom(scan_sig, scan_bkg)
            total_fom += fom

            if debug:
                print("--------------------------------------------------")
                print(f"[BIN {i:2d}] [{x_low:.3f}, {x_high:.3f}]")
                print(f"  Run2: S={run2_sig:.4f}  B={run2_bkg:.4f}  F={run2_fake:.4f}")
                print(f"  FOM = {fom:.6f}")

                for era, s, b, f in per_era_info:
                    print(f"    {era}: S={s:.4f} B={b:.4f} F={f:.4f}")

    if debug:
        print("==================================================")
        print(f"TOTAL FOM = {total_fom:.6f}")
        print("==================================================")
...
def build_cumsum_cache(bin_cache, all_edges):
    ...
    edge_grid = np.round(np.arange(-0.4, 0.401, BIN_STEP) + BIN_STEP/2, 5)
...
def find_tail_cut(histograms, n_consecutive=0):
    ...
    test_edges =  np.arange(0.4, -0.4, -BIN_STEP)
...
seed = 100000 * int(mass) + args.job_id
...
os.makedirs("logs_coarse", exist_ok=True)
log_name = f"logs_coarse/scan_{mass}_{flavour}_job{args.job_id}.txt"
...
os.makedirs("results_coarse", exist_ok=True)

np.save(
    f"results_coarse/results_{mass}_{flavour}_job{args.job_id}.npy",
    {
        "mass": mass,
        "flavour": flavour,
        "job_id": args.job_id,
        ...
        "fom_coarse": float(fom_scan_run2),
        "edges_coarse": [round(x, 5) for x in edges_run2],
