...
import numpy as np
import random
from dp_binning import dp_optimal_binning,fix_fake_and_bkg
...
os.makedirs("logs_dp", exist_ok=True)
log_name = f"logs_dp/scan_{mass}_{flavour}_job{args.job_id}.txt"
...
for era, (h, hs, hf) in histograms.items():
    ...
    for k in range(1, h.GetNbinsX()+1):
        ...
        centers.append(x)
        bvals.append(h.GetBinContent(k))
        fvals.append(hf.GetBinContent(k))
        svals.append(hs.GetBinContent(k))
...
all_edges = np.round(np.arange(-0.4, 0.40+BIN_STEP , BIN_STEP), 5)

bin_cache = build_cumsum_cache(bin_cache, all_edges)

edges_dp, fom_dp = dp_optimal_binning(
...
edges_greedy, fom_greedy = greedy_optimize_edges(
    ...
    tail_cut=0.4,   # or your computed tail_cut
...
np.save(
    f"results_dp/results_{mass}_{flavour}_job{args.job_id}.npy",
    {
        "mass": mass,
        "flavour": flavour,
        "job_id": args.job_id,
        ...
        # --- FOMs ---
        "fom_dp": float(fom_dp),
        "fom_greedy": float(fom_greedy),
        "fom_best": float(fom_greedy),
        "fom_existing": float(fom_existing),

        # --- binning ---
        "edges_dp": [round(x, 5) for x in edges_dp],
        "edges_best": [round(x, 5) for x in edges_greedy],
        "edges_greedy": [round(x, 5) for x in edges_greedy],
...
print(" FINAL RESULT")
print("==============================")
print("Best FOM Run2(scan):      ", round(fom_greedy,5))
print("Best edges:           ", format_edges_str(edges_greedy))
print("")
