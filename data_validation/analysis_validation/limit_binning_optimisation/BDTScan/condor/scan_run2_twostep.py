...
def safe_cumsum(cumsum, i):

    return cumsum[i] if i >= 0 else 0.0
...
def build_cumsum_cache(bin_cache):
    ...
    edge_grid = np.round(
        ...
        np.arange(-0.4, 0.4 + BIN_STEP, BIN_STEP),
...
def evaluate_combination_fnct(args):
    ...
    for j in range(nbins):
        ...
        for era, (centers, cumsum_b, cumsum_f, cumsum_s, edge_indices) in bin_cache.items():
            ...
            bkg  = safe_cumsum(cumsum_b, i_high-1) - safe_cumsum(cumsum_b, i_low-1)
            fake = safe_cumsum(cumsum_f, i_high-1) - safe_cumsum(cumsum_f, i_low-1)
            sig  = safe_cumsum(cumsum_s, i_high-1) - safe_cumsum(cumsum_s, i_low-1)
...
def build_edges(Xcut):
    ...
    tmp_edges_low = [-0.05, 0, 0.05]
    tmp_edges_medium = np.arange(0.07, 0.15, 0.02)
    tmp_edges_high = np.arange(0.15, 0.4, 0.01)
    tmp_edges_all = np.concatenate((tmp_edges_low, tmp_edges_medium, tmp_edges_high))
...
def scan_optimal_variable_binning(scan_era,histograms, nbins, max_trials, flav=None):
    ...
    all_edges = np.round(np.arange(-0.4, 0.4 + BIN_STEP, BIN_STEP), 5)
    ...
    print(f"All bins : {format_edges_str(edges_simple)}")
    ...
    print(f"[INFO] combinations after cut: {ncomb_simple_after:.3e}")
    ...
    total_trials =  n_trials_simple_eff
    ...
    return best_edges, best_fom, bin_cache,tail_cut
            

def refine_edges_locally(
    best_edges,
    bin_cache,
    nbins,
    x_min,
    ...
    print("\n[REFINE] Starting local refinement")

    best_edges = list(best_edges)
    best_fom, _ = evaluate_combination_fnct(
        (best_edges[1:-1], bin_cache, nbins, x_min, x_max, tail_cut, scan_era)
    )

    print(f"[REFINE] Initial FOM = {best_fom:.6f}")

    for iteration in range(max_iter):

        improved = False

        print(f"\n[REFINE] Iteration {iteration+1}")

        # loop over movable edges (exclude first/last)
        for i in range(1, len(best_edges) - 1):

            original_edge = best_edges[i]

            # define scan window
            low  = max(best_edges[i-1] + step, original_edge - window)
            high = min(best_edges[i+1] - step, original_edge + window)

            candidates = np.arange(low, high + step, step)

            local_best_edge = original_edge
            local_best_fom = best_fom

            for new_edge in candidates:
                trial_edges = best_edges.copy()
                new_edge = round(round(new_edge / BIN_STEP) * BIN_STEP, 5)
                trial_edges[i] = new_edge

                result = evaluate_combination_fnct(
                    (trial_edges[1:-1], bin_cache, nbins, x_min, x_max, tail_cut, scan_era)
                )
                ...
                if fom > local_best_fom:
                    local_best_fom = fom
                    local_best_edge = new_edge

            # apply best move
            if local_best_edge != original_edge:
                print(f"[REFINE] Edge {i} moved: {original_edge:.4f} -> {local_best_edge:.4f}  (FOM: {local_best_fom:.5f})")

                best_edges[i] = round(local_best_edge, 5)

                best_fom = local_best_fom
                improved = True

        if not improved:
            print("[REFINE] Converged (no improvement)")
            break

    print("\n[REFINE] Final result")
    print("Edges:", format_edges_str(best_edges))
    print("FOM  :", best_fom)
...
def snap_edges_to_grid(edges, step=0.005, min_width=0.002):

    snapped = [round(round(x / step) * step, 5) for x in edges]

    # enforce ordering / avoid collapsed bins
    for i in range(1, len(snapped)):
        if snapped[i] <= snapped[i-1] + min_width:
            snapped[i] = round(snapped[i-1] + step, 5)
...
os.makedirs("logs_twostep", exist_ok=True)
log_name = f"logs_twostep/scan_twostep_{mass}_{flavour}_job{args.job_id}.txt"
...
for era in eras:
    ...
    print(f"[CHECK] Signal integral ({era}):", h_sig.Integral())
    
    print(f"[CHECK] Bkg integral ({era}):", h_bkg.Integral())
...
edges_run2, fom_scan_run2, bin_cache_run2, tailcut = scan_optimal_variable_binning(
...
edges_refined, fom_refined = refine_edges_locally(
    edges_run2,
    bin_cache_run2,
    nbins=args.n_bins,
    x_min=-0.4,
...
edges_refined2, fom_refined2 = refine_edges_locally(
    edges_refined,
    bin_cache_run2,
    nbins=args.n_bins,
    x_min=-0.4,
...
edges_snapped = snap_edges_to_grid(edges_refined2, step=0.005)

result = evaluate_combination_fnct(
    (edges_snapped[1:-1], bin_cache_run2, args.n_bins, -0.4, 0.4, tailcut, "Run2")
)

if result is not None:
    fom_snapped, _ = result
else:
    print("[WARNING] Snapped edges invalid, keeping original")
    edges_snapped = edges_refined2
    fom_snapped = fom_refined2

print(f"Snapped edges : {format_edges_str(edges_snapped)}")
print(f"fom_snapped   = {fom_snapped}")
...
os.makedirs("results_twostep", exist_ok=True)

np.save(
    f"results_twostep/results_{mass}_{flavour}_job{args.job_id}.npy",
    {
        "mass": mass,
        "flavour": flavour,
        "job_id": args.job_id,
        ...
        # --- FOMs ---
        "fom_best": float(fom_refined2),
        "fom_snapped": float(fom_snapped),
        "fom_twostep": float(fom_refined2),
        "fom_existing": float(fom_existing),

        # --- binning ---
        "edges_best": [round(x, 5) for x in edges_refined2],
        "edges_snapped": [round(x, 5) for x in edges_snapped],

        # --- metadata ---
        "n_bins": args.n_bins,
        "n_trials": args.n_trials,
        "scan_type": "Run2_twostep",
        "bin_step": BIN_STEP,
        "final_step": 0.005,
        "seed": seed,
    }
...
print("Snapped FOM:     ", round(fom_snapped,5))

print("Snapped edges:   ", format_edges_str(edges_snapped))
...
print(" FINAL RESULT")
print("==============================")
print("Scan FOM:        ", round(fom_scan_run2,5))

print("Refined FOM:     ", round(fom_refined,5))

print("Refined2 FOM:    ", round(fom_refined2,5))
print("Final FOM:    ", round(fom_snapped,5))

print("Final edges:     ", format_edges_str(edges_snapped))
print("")
