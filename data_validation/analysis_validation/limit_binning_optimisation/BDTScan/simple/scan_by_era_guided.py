...
def build_edges(Xcut):
    ...
    tmp_edges_med  = np.arange(0.0, 0.2, 0.02)

    tmp_edges_high = np.arange(0.2, 0.4, 0.01)
...
def build_edges_from_tailcut(Xcut):
    ...
    edges_low = [-0.2, -0.15, -0.1, -0.05]

    # --- case 1: very tight tail ---
    if Xcut < 0.25:

        edges_med = np.arange(0.0, 0.06, 0.02)
        edges_high = np.arange(0.06, Xcut + 0.005, 0.01)
    ...
    # --- case 2: medium tail ---
        ...
        edges_high = np.arange(0.10, Xcut + 0.005, 0.01)
    ...
    # --- case 3: long tail (your main case) ---
        ...
        edges_vhigh = np.arange(Xcut - 0.10, Xcut + 0.005, 0.01)
...
def scan_optimal_variable_binning(scan_era,histograms, nbins, max_trials, flav=None):
    ...
    tail_cut = find_tail_cut(histograms) + 0.005
...
log_name = f"logs_update/scan_log_mass{mass}_{flavour}.txt"
