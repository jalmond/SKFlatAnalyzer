...
def create_dummy_pads(bin_labels, y_max):
    ...
    dummy_down.GetYaxis().SetRangeUser(0.,2.)
...
def MakeDataHist(years, channels,data_files, input_dir, hist_base,hist_config):
    """
    ...
    for year_idx, year in enumerate(years):
        for channel_idx, channel in enumerate(channels):
            ...
            hist_path = f"{hist_base}/{hist_config}/{channel}/NObj/N_BJet"
...
def process_histograms(years, channels,input_dir, background_files, hist_base,hist_config):
    """
    ...
    for year_idx, year in enumerate(years):
        for channel_idx, channel in enumerate(channels):
            ...
            for bkg_file, label in background_files:
                ...
                d_err = err[0]  # start with statistical error

                # Custom treatment for WZ uncertainty using WZAlt
                if "WZ" in bkg_file:
                    alt_file_path = os.path.join(input_dir, year, "HNL_ControlRegionLLL_WZAlt.root")
                    alt_file = ROOT.TFile.Open(alt_file_path, "READ")
                    if alt_file and not alt_file.IsZombie():
                        alt_hist = alt_file.Get(hist_path)
                        if alt_hist:
                            alt_norm = alt_hist.Integral(1, alt_hist.GetNbinsX())
                            wz_diff = abs(norm - alt_norm)
                            d_err = math.sqrt(d_err**2 + wz_diff**2 + (0.2 * norm)**2)
                        alt_file.Close()
                else:
                    # Use default systematics
                    uncer = 0.1
                    if "WW" in file_path:
                        uncer = 0.2
                    if "VVV" in file_path:
...
def compute_and_apply_wz_sf_llllbins(h_Data, histograms, args, output_sf_file="wz_scale_factors.txt"):
    bin_map = {
        "2016preVFP": 4,
        "2016postVFP": 8,
        "2017": 12,
    ...
    for key in histograms.keys():
        if args.usewz_alt:
            if "WZAlt" in key:
                ...
                if "WZ" in key and "Alt" not in key:
                    wz_file_key = key
                    break
                
    if not wz_file_key:
        logging.error("WZ histogram not found in histograms.")
        return

    wz_hist = histograms[wz_file_key]

    with open(output_sf_file, "w") as f_out:
        f_out.write("# WZ Scale Factors for each era (LLLL bin matching)\n")
        f_out.write("# Format: Year  ScaleFactor\n")

        for year, bin_idx in bin_map.items():
            bin_i = bin_idx

            data_val = h_Data.GetBinContent(bin_i)
            other_bkg_val = 0.
            wz_val = wz_hist.GetBinContent(bin_i)

            for key, hist in histograms.items():
                if key == wz_file_key:
                    continue
                other_bkg_val += hist.GetBinContent(bin_i)

            denom = wz_val
            numer = max(data_val - other_bkg_val, 0.)

            if denom <= 0:
                logging.warning("WZ yield is zero in %s, skipping SF." % year)
                continue

            sf = numer / denom
            logging.info("WZ Scale Factor for %s (LLLL bin): %.3f" % (year, sf))

            old_val = wz_hist.GetBinContent(bin_i)
            old_err = wz_hist.GetBinError(bin_i)

            new_val = sf * old_val
            new_err = sf * old_err

            wz_hist.SetBinContent(bin_i, new_val)
            wz_hist.SetBinError(bin_i, new_err)

            f_out.write("%-12s %.5f\n" % (year, sf))

    print("WZSF scale factors saved to: %s" % output_sf_file)
...
regions = ["HNL_WZ_SR1_ThreeLepton_CR","HNL_WZVBF_ThreeLepton_CR","HNL_WZB_ThreeLepton_CR","HNL_WZ_SR3_ThreeLepton_CR"]
...
try:
    ...
    for hist_base, hist_config, flavour, channels in hist_bases:
        ...
        if Run:
            ...
            # Loop over each year and channel, extract the corresponding histogram
            h_Data = MakeDataHist(years, channels, data_files,input_dir,hist_base,hist_config)
            histograms = process_histograms(years, channels, input_dir, background_files, hist_base,hist_config)

            # Apply WZ SFs so LLLL bin (bin 4,8,12,16) matches Data
            compute_and_apply_wz_sf_llllbins(
                h_Data,
                histograms,
                args,
                output_sf_file=f"results_lll_ratio/{Plot_Version}/WZ_SFs_{hist_base}_{hist_config}.txt"
            )
