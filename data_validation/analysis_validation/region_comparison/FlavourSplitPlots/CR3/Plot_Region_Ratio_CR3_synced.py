...
def make_legend(gr_Data_dummy,background_files):
    ...
    # Add a legend                                                                                                                                  
    legend = ROOT.TLegend(0.2, 0.3, 0.35, 0.8)
    ...
    for year_idx, year in enumerate(years):
        for channel_idx, channel in enumerate(channels):
            ...
            for bkg_file, label in background_files:
                ...
                uncer = 0.2
                if "W^{#pm}W^{#pm}" in file_path:
                    uncer = 0.2

                if "ZZ" in file_path:
                    uncer = 0.15
                
                if "MisIDe" in file_path:
                    uncer = 0.3
                if "Charge" in file_path:
                    uncer = 0.25
...
background_files = [
    ["HNL_ControlRegionOne_CF.root", "#splitline{Charge flip}{background}"],
    ["HNL_ControlRegionOne_ZG.root", "Z#gamma"],
    ["HNL_ControlRegionOne_XConv.root", "W/VV/t #plus #gamma"],
    ["HNL_ControlRegionOne_Fake.root", "#splitline{MisId. Lepton}{background}"],
    ["HNL_ControlRegionOne_Top_Higgs.root", "Top,Higgs,VVV"],
    ["HNL_ControlRegionTwo_WZ.root", "WZ"],
    ["HNL_ControlRegionTwo_WW.root", "W^{#pm}W^{#pm}"],
    ["HNL_ControlRegionTwo_ZZ.root", "ZZ"]
]


colors = {
    "#splitline{Charge flip}{background}": ROOT.kYellow,
    "W/VV/t #plus #gamma": ROOT.kMagenta-9,
    "Z#gamma": ROOT.kViolet + 1,
    "#splitline{MisId. Lepton}{background}":  ROOT.kAzure + 8,
    "Top,Higgs,VVV": ROOT.kRed,
    "W^{#pm}W^{#pm}": ROOT.kOrange,
    "WZ": ROOT.kGreen + 1,
    "ZZ":  ROOT.kGreen+3,
}
...
regions = ["HNL_HighMassSR3_InvBJet_CR","HNL_HighMassSR3_InvMET_CR","HNL_HighMassSR2_Inclusive"]
...
try:
    ...
    for hist_base, hist_config, flavour, channels in hist_bases:
        ...
        if Run:
            ...
            # Add a legend                                                                                                                                                                                                                          
            legend = ROOT.TLegend(0.2, 0.35, 0.35, 0.8)
            legend.SetBorderSize(0)
            legend.SetFillStyle(0)
            legend.SetTextSize(0.03)
