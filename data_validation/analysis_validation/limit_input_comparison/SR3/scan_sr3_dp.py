...
def evaluate_binning_for_masses(selected_dir, era, flavour, base_path, hname,
    ...
    for m in masses:
        ...
        sig_hist = f_sig_tmp.Get(f"{base_path}/HNL_ULIDv2/{flavour}/Mass/{hname}")
        if not sig_hist:
            f_sig_tmp.Close()
            continue
        _, _, _, sig_arr = extract_edges_and_arrays(f_bkg.Get(f"{base_path}/HNL_ULIDv2/{flavour}/Mass/{hname}"), sig_hist)
...
def main():
    ...
    path = f"PassSR3/HNL_ULIDv2/{args.flavour}/Mass/"
    hname = "DiJet_M_l1jj"
    ...
    with open(log_file, 'w') as log:
        ...
        if best_edges is not None:
            ...
            evaluate_binning_for_masses(selected_dir, args.era, args.flavour, "PassSR3", hname,
