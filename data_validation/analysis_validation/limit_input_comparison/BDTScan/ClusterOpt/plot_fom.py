...
# Compile regex
result_pattern = re.compile(r"\[RESULT\] Best total FOM: ([\d.]+)")
...
def extract_v2_fom(mass, flav, era):
    summary_file = f"logs/scan_{mass}_{flav}_{era}_35_30.out"
    ...
    for flav in flavours:
        for era in eras:
            ...
            for graph, label in graphs:
                graph.GetXaxis().SetLimits(5, 35)  # Set x-axis range
            ...
            if v2_fom:
                line = ROOT.TLine(5, v2_fom, 35, v2_fom)
                line.SetLineStyle(2)
            ...
            c.SetGrid()
            legend.Draw()
            c.SaveAs(f"{out_dir}/FOMScan_{mass}_{flav}_{era}.pdf")
