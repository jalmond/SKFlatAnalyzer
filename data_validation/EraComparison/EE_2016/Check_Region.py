...
for ID in id_versions:
    for hist_base,flavour,channels in hist_bases:
        Path(f"results_{flavour}").mkdir(exist_ok=True)
        ...
        era_bins = [0, 1, 2, 3]
        era_offsets = {base_channels[0]: -0.2, base_channels[1]: 0.0, base_channels[2]: 0.2}
        colors = {base_channels[0]: ROOT.kRed+1, base_channels[1]: ROOT.kBlue+1, base_channels[2]: ROOT.kGreen+2}
        markers = {base_channels[0]: 20, base_channels[1]: 21, base_channels[2]: 22}
        ...
        canvas.Print(f"results_{flavour}/{hist_base}_{ID}_TotalBackground_AllChannels.png")
        canvas.Print(f"results_{flavour}/{hist_base}_{ID}_TotalBackground_AllChannels.pdf")
