...
for y in years:
    ...
    for m in mass:
        ...
        for c in Channel:
            ...
            noMerge=False
            nFiles=0
            hadd = "hadd " + outpath + "HNtypeI_JA_HN_S_Tchannel_" + c[0] + "_" + m + "_nlo.root "
            s_cp = outpath + "HNtypeI_JA_HN_S_Tchannel_" + c[0] + "_" + m + "_nlo.root "
            counter=0
            sigs=[]
            for s in c[1]:
                for ch in charge:
            ...
            if len(sigs) == 1:
                hadd = "cp " + sigs[0] +" " + s_cp
            if len(sigs)==0:
                print "No Signal " + outpath + "HNtypeI_JA_HN_S_Tchannel_" + c[0] + "_" + m + "_nlo.root "
                continue 
            print " "
            print hadd
            if os.path.exists(outpath + "HNtypeI_JA_HN_S_Tchannel_" + c[0] + "_" + m + "_nlo.root"):
                os.system("rm " + outpath + "HNtypeI_JA_HN_S_Tchannel_" + c[0] + "_" + m + "_nlo.root")
            os.system(hadd)
