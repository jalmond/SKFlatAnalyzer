...
Particle AnalyzerCore::UpdateMETSmearedFatJet(const Particle& METv, const std::vector<FatJet>& fatjets){

  double met_x = METv.Px();
  double met_y = METv.Py();

  double px_orig(0.), py_orig(0.),px_corrected(0.), py_corrected(0.);
  for(auto jet : fatjets){
    px_orig+= jet.PxUnSmeared();
    py_orig+= jet.PyUnSmeared();
...
Particle AnalyzerCore::UpdateMETSyst(AnalyzerParameter param, const Particle& METv,  std::vector<Electron> electrons){
  ...
  if(param.syst_ ==AnalyzerParameter::ElectronEnUp ||
    ...
    for(unsigned int i=0; i<electrons.size(); i++){
      ...
      px_orig+= electrons.at(i).MiniAODPt()*TMath::Cos(electrons.at(i).Phi());
      py_orig+= electrons.at(i).MiniAODPt()*TMath::Sin(electrons.at(i).Phi());
