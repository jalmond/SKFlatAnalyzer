#include "HNtypeI_opt_mm.h"

void HNtypeI_opt_mm::initializeAnalyzer(){
  ...
  cout << "[HNtypeI_opt_mm::initializeAnalyzer()] RunFake = " << RunFake << endl;
  cout << "[HNtypeI_opt_mm::initializeAnalyzer()] RunCF = " << RunCF << endl;
  cout << "[HNtypeI_opt_mm::initializeAnalyzer()] RunSyst = " << RunSyst << endl;
  cout << "[HNtypeI_opt_mmr::initializeAnalyzer()] PromptLeptonOnly = " << PromptLeptonOnly << endl;
  ...
  if(!IsDATA){
    ...
    cout << "[HNtypeI_opt_mm::initializeAnalyzer()] PUfname = " << PUfname << endl;
    cout << "[HNtypeI_opt_mm::initializeAnalyzer()] PUhname = " << PUhname << endl;
    cout << "[HNtypeI_opt_mm::initializeAnalyzer()] Signal = " << Signal << endl;
...
void HNtypeI_opt_mm::executeEvent(){
  ...
  for(unsigned int it_id=0; it_id<ELIDs.size(); it_id++){
    ...
    param.Name = "HNtypeI_opt_mm_"+ElectronTightID;
	...
	for(unsigned int idz=0; idz < v_dz.size();idz++){
	  ...
	  //************************************************///
	  param.Name = "HNtypeI_opt_mm_"+ElectronTightID+"_"+s_cc+v_dx[idx]+v_dz[idz];
	  
	  //************************************************///
	  // run event
	  //************************************************///
	  executeEventFromParameter(param, ElectronTightID,ElectronLooseID, s_cc, d_dx_b, d_dx_e, d_dz_b, d_dz_e,999.,999.);
	}
      }
    }
  } 
...
void HNtypeI_opt_mm::executeEventFromParameter(AnalyzerParameter param, TString El_ID, TString loose_id, TString s_cc, double dx_b, double dx_e, double dz_b, double dz_e, double iso_b, double iso_e){
  ...
  //************************************************///
  // only plot CR for POG IP cuts
  //************************************************///
  if (dx_b == 0.05 && dx_e==0.1 && dz_b == 0.1 && dz_e == 0.2) plot_CR=true;
  ...
  if(nom_ID)FillCutFlow(IsCentral, "CutFlow", "NoCut_"+param.Name, weight);
  if(nom_ID)FillCutFlow(IsCentral, "CutFlow", "NoCut_no_weight_"+param.Name, 1.);
  ...
  if(nom_ID)FillCutFlow(IsCentral, "CutFlow", "METFilter_"+param.Name, weight);
  ...
  std::vector<Electron> myelectrons = SelectElectrons(this_AllElectrons, El_ID, param.Electron_MinPt, 2.4, cc, dx_b, dx_e, dz_b, dz_e ,999.,999.);
  if(RunFake)  myelectrons = SelectElectrons(this_AllElectrons, loose_id, param.Electron_MinPt, 2.4, cc, 0.2,0.2,0.5,0.5,999.,999.);
  ...
  //**************/data6/Users/jalmond/SKFlatRunlog/2020_08_31_210519__862799__HNtypeI_opt_mm__Year2016__SkimTree_Dilepton__TAMSA1/DoubleEG_periodB_ver2**********************************///   
  ...
  if(llCand.M() < 100. && llCand.M() > 60.){
    ...
    TString ev_type="";
    if(RunFake) {
      bool el1_tight=myelectrons[0].PassID(El_ID, cc, dx_b, dx_e, dz_b, dz_e,999.,999.);
      bool el2_tight=myelectrons[1].PassID(El_ID, cc, dx_b, dx_e, dz_b, dz_e,999.,999.);
...
void HNtypeI_opt_mm::MakeSignalPlots(){
  bool mupt_pass=true;
  int mother_nu(0);
...
void HNtypeI_opt_mm::FillSigRegionPlots1(TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7, double var8, double var9, double var10, double var11){
  ...
  FillHist( label_1+ "/"+ label_1 +  "_reco_mlljj_"  + label_2,  W1cand.M(),  w, 200, 0, 2000, "Reco M_{lljj}");
  FillHist( label_1+ "/"+ label_1 +  "_reco_ml1jj_"  + label_2,  N1cand.M(),  w, 200, 0, 2000, "Reco M_{l1jj}");
...
void HNtypeI_opt_mm::FillSigRegionPlots2(TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7){
  ...
  FillHist( label_1+ "/"+ label_1 +  "_reco_mllJ_"  + label_2,  W1cand.M(),  w, 200, 0, 2000, "Reco M_{lljj}");
  FillHist( label_1+ "/"+ label_1 +  "_reco_ml1J_"  + label_2,  N1cand.M(),  w, 200, 0, 2000, "Reco M_{l1jj}");
  FillHist( label_1+ "/"+ label_1 +  "_reco_ml2J_"  + label_2,  N2cand.M(),  w, 200, 0, 2000, "Reco M_{l2jj}");
  FillHist( label_1+ "/"+ label_1 +  "_njets_" + label_2, jets.size() , w, 10, 0., 10., "N_{jets}");

}
void HNtypeI_opt_mm::FillRegionPlots(bool plotCR, TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w){
...
double HNtypeI_opt_mm::MT(Particle a, Particle b){
...
std::vector<Jet> HNtypeI_opt_mm::JetsVetoLeptonInside(std::vector<Jet> jets, std::vector<Electron *> els, std::vector<Muon *> mus, double dR){
...
bool HNtypeI_opt_mm::LeptonPassID(Lepton &lepton, TString ID){
  ...
  if(lepton.LeptonFlavour()==Lepton::ELECTRON){
    ...
    cerr << "[HNtypeI_opt_mm::LeptonPassID] lepton flavour wrong.." << endl;
    exit(EXIT_FAILURE);
    return false;
  }
...
void HNtypeI_opt_mm::FillWeightHist(TString label, double _weight){
  FillHist( "weights/"+ label , _weight ,1., 200, 0., 5,"ev weight");
}
void HNtypeI_opt_mm::FillCutFlow(bool IsCentral, TString suffix, TString histname, double weight){
...
double HNtypeI_opt_mm::LSFSF(int lepflav, int dir){
  ...
  if(lepflav==0){
    ...
    cerr << "[HNtypeI_opt_mm::LSFSF] wrong lepflav : " <<  lepflav << endl;
    exit(EXIT_FAILURE);
    return 1.;
  }
