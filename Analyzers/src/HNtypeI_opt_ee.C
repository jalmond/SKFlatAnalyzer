#include "HNtypeI_opt_ee.h"

void HNtypeI_opt_ee::initializeAnalyzer(){
  ...
  cout << "[HNtypeI_opt_ee::initializeAnalyzer()] RunFake = " << RunFake << endl;
  cout << "[HNtypeI_opt_ee::initializeAnalyzer()] RunCF = " << RunCF << endl;
  cout << "[HNtypeI_opt_ee::initializeAnalyzer()] RunSyst = " << RunSyst << endl;
  cout << "[HNtypeI_opt_eer::initializeAnalyzer()] PromptLeptonOnly = " << PromptLeptonOnly << endl;
  ...
  if(!IsDATA){
    ...
    cout << "[HNtypeI_opt_ee::initializeAnalyzer()] PUfname = " << PUfname << endl;
    cout << "[HNtypeI_opt_ee::initializeAnalyzer()] PUhname = " << PUhname << endl;
    cout << "[HNtypeI_opt_ee::initializeAnalyzer()] Signal = " << Signal << endl;
...
void HNtypeI_opt_ee::executeEvent(){
  ...
  ELIDs.push_back("passMVAID_noIso_WP90");
  ELIDs.push_back("passMVAID_noIso_WP80");
  ...
  for(unsigned int it_id=0; it_id<ELIDs.size(); it_id++){
    ...
    else    if(ELIDs.at(it_id).Contains("HNTight2016")){
	ElectronLooseID = "HNMVALoose";
	ElectronVetoID  = "HNVeto2016";
      }
    else {
    ...
    param.Name = "HNtypeI_opt_ee_"+ElectronTightID;
    ...
    vector<TString> v_dx;
    if(!ElectronTightID.Contains("noIso")) { 
    ...
    vector<TString> v_iso;
    if(ElectronTightID.Contains("noIso")) { 
      v_iso.push_back("b0.07_ec0.05");
      v_iso.push_back("b0.07_ec0.06");
      v_iso.push_back("b0.07_ec0.07");
      v_iso.push_back("b0.08_ec0.05");
      v_iso.push_back("b0.08_ec0.06");
      v_iso.push_back("b0.08_ec0.07");
      v_iso.push_back("b0.08_ec0.08");
      v_iso.push_back("b0.09_ec0.09");
      v_iso.push_back("b0.09_ec0.05");
      v_iso.push_back("b0.10_ec0.10");
      v_iso.push_back("b0.10_ec0.05");
      v_iso.push_back("b999_ec999");
    }
    else v_iso.push_back("b999_ec999");
    
    //************************************************///
    // loop over charge cut values 0/1
    ///************************************************///
      if(ElectronTightID == "HNTight2016"){
	param.Name = "HNtypeI_opt_ee_"+ElectronTightID;
	
	//************************************************///                                                                                                              
	// run event                                                                                                                                                       
	//************************************************///                                                                                                              
	executeEventFromParameter(param, ElectronTightID,ElectronLooseID, "", 999.,999.,999.,999.,999.,999.);
	
      }
      else{
	for(int icc=0; icc < 2; icc++){
	  TString s_cc = (icc==0) ? "chargeconst" : "" ;
	  ...
	  for(unsigned int idx=0; idx < v_dx.size();idx++){
	    ...
	    for(unsigned int idz=0; idz < v_dz.size();idz++){
	      ...
	      for(unsigned int iiso=0; iiso < v_iso.size();iiso++){
		//************************************************///                                                                                                               
		// loop over vector of IP Z values                                                                                                                                  
		//************************************************///                                                                                                               
		vector<TString> v_iso;
		...
		double d_iso_b(-999.), d_iso_e(-999.);
		if(v_iso[iiso].Contains("b0.07")) d_iso_b= 0.07;
		if(v_iso[iiso].Contains("b0.08")) d_iso_b= 0.08;
		if(v_iso[iiso].Contains("b0.09")) d_iso_b= 0.09;
		if(v_iso[iiso].Contains("b0.10")) d_iso_b= 0.1;
		if(v_iso[iiso].Contains("b999")) d_iso_b= 999.;
		if(v_iso[iiso].Contains("ec0.05")) d_iso_e= 0.05;
		if(v_iso[iiso].Contains("ec0.06")) d_iso_e= 0.06;
		if(v_iso[iiso].Contains("ec0.07")) d_iso_e= 0.07;
		if(v_iso[iiso].Contains("ec0.08")) d_iso_e= 0.08;
		if(v_iso[iiso].Contains("ec0.09")) d_iso_e= 0.09;
		if(v_iso[iiso].Contains("ec0.10")) d_iso_e= 0.10;
		if(v_iso[iiso].Contains("ec999")) d_iso_e= 999.;
		
		//************************************************///
		// change param name to link to IP/CC cuts
		//************************************************///
		param.Name = "HNtypeI_opt_ee_"+ElectronTightID+"_"+s_cc+v_dx[idx]+v_dz[idz] + v_iso[iiso];
		
		//************************************************///
		// run event
		//************************************************///
		executeEventFromParameter(param, ElectronTightID,ElectronLooseID, s_cc, d_dx_b, d_dx_e, d_dz_b, d_dz_e, d_iso_b,d_iso_e);
	      }
	    }
	  }
      }
...
void HNtypeI_opt_ee::executeEventFromParameter(AnalyzerParameter param, TString El_ID, TString loose_id, TString s_cc, double dx_b, double dx_e, double dz_b, double dz_e, double iso_b, double iso_e){
  
  bool cc = false;
  if(s_cc=="chargeconst") cc=true;
  ...
  std::vector<Electron> myelectrons = SelectElectrons(this_AllElectrons, El_ID, param.Electron_MinPt, 2.4, cc, dx_b, dx_e, dz_b, dz_e, iso_b, iso_e );
  if(RunFake)  myelectrons = SelectElectrons(this_AllElectrons, loose_id, param.Electron_MinPt, 2.4, cc, 0.2,0.2,0.5,0.5,0.6,0.6);
  ...
  if(!IsDATA){
    ...
    for(unsigned int i=0; i<myelectrons.size(); i++){
      ...
      double this_idsf = 1.;//mcCorr->ElectronID_SF(Electron_ID_SF_Key, myelectrons.at(i).scEta(), myelectrons.at(i).Pt(), SystDir_ElectronIDSF);
  ...
  //**************/data6/Users/jalmond/SKFlatRunlog/2020_08_31_210519__862799__HNtypeI_opt_ee__Year2016__SkimTree_Dilepton__TAMSA1/DoubleEG_periodB_ver2**********************************///   
  ...
  if(charge_s ==  "opposite_sign")  return;
  //************************************************///   
  ...
  if(!(llCand.M() < 100. && llCand.M() > 60.)){
    ...
    TString ev_type="";
    if(RunFake) {
      bool el1_tight=myelectrons[0].PassID(El_ID, cc, dx_b, dx_e, dz_b, dz_e,iso_b, iso_e);
      bool el2_tight=myelectrons[1].PassID(El_ID, cc, dx_b, dx_e, dz_b, dz_e, iso_b, iso_e);
...
void HNtypeI_opt_ee::MakeSignalPlots(){
  bool mupt_pass=true;
  int mother_nu(0);
...
void HNtypeI_opt_ee::FillSigRegionPlots1(TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7, double var8, double var9, double var10, double var11){
  ...
  FillHist( label_1+ "/"+ label_1 +  "_reco_mlljj_"  + label_2,  W1cand.M(),  w, 20, 0, 2000, "Reco M_{lljj}");
  FillHist( label_1+ "/"+ label_1 +  "_reco_ml1jj_"  + label_2,  N1cand.M(),  w, 20, 0, 2000, "Reco M_{l1jj}");
  //FillHist( label_1+ "/"+ label_1 +  "_reco_ml2jj_"  + label_2,  N2cand.M(),  w, 200, 0, 2000, "Reco M_{l2jj}");
  //FillHist( label_1+ "/"+ label_1 +  "_njets_" + label_2, jets.size() , w, 10, 0., 10., "N_{jets}");

}

void HNtypeI_opt_ee::FillSigRegionPlots2(TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7){
  ...
  //FillHist( label_1+ "/"+ label_1 +  "_reco_ml2J_"  + label_2,  N2cand.M(),  w, 200, 0, 2000, "Reco M_{l2jj}");
  //FillHist( label_1+ "/"+ label_1 +  "_njets_" + label_2, jets.size() , w, 10, 0., 10., "N_{jets}");

}
void HNtypeI_opt_ee::FillRegionPlots(bool plotCR, TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w){
  ...
  if(jets.size() ==1 ) lljjCand += jets[0];
    ...
    if(els.size()> 0)FillHist( label_1+ "/"+ label_1 +  "_reco_ml1jj_"  + label_2,   (els[0] + jets[m]+jets[n]).M(),  w, 250, 0, 1000, "Reco M_{l1jj}");
    if(els.size()> 1){
      FillHist( label_1+ "/"+ label_1 +  "_reco_ml2jj_"  + label_2,   (els[1] + jets[m]+jets[n]).M(),  w, 250, 0, 1000, "Reco M_{l2jj} ");
      FillHist( label_1+ "/"+ label_1 +  "_reco_mlljj_"  + label_2,  lljjCand.M(),  w, 250, 0, 1000, "Reco M_{lljj}");
    }
  }
  ...
  for(unsigned int i=0; i < els.size(); i++){
    ...
    if(i==1) el_lab="el2";
    if(i==2) el_lab="el3";
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_pt_" + label_2  , els.at(i).Pt() , w, 500, 0., 1000., "el p_{T} GeV");
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_dxy_" + label_2  , els.at(i).dXY() , w, 500, -1., 1., "dXY");
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_dz_" + label_2  , els.at(i).dZ() , w, 500, -1., 1., "dZ");
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_reliso_" + label_2  , els.at(i).RelIso() , w, 50, 0., 1., "R_{ISO} GeV");
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_eta_"+ label_2  , els.at(i).Eta() , w, 60, -3., 3.,"el #eta");
    FillHist( label_1+ "/"+ label_1 +  "_"+el_lab+"_mt_" + label_2, MT(met, els[i])  , w, 200, 0., 400.,"M_{T} GeV");
    FillHist( label_1+ "/"+ label_1 +  "_"+el_lab+"_cc_" + label_2, els[i].IsGsfCtfScPixChargeConsistent()  , w, 2, 0., 2.,"Charge Cons.");
    FillHist( label_1+ "/"+ label_1 +  "_"+el_lab+"_convveto_" + label_2, els[i].PassConversionVeto()  , w, 2, 0., 2.,"Conv. veto.");
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_mva_" + label_2  , els.at(i).MVAIso() , w, 100, -1., 1., "el mva");
    FillHist( label_1+ "/"+ label_1 + "_"+el_lab+"_mvanoniso_" + label_2  , els.at(i).MVANoIso() , w, 100, -1., 1., "el mva nonIso");
  }
  return;
}

double HNtypeI_opt_ee::MT(Particle a, Particle b){
...
std::vector<Jet> HNtypeI_opt_ee::JetsVetoLeptonInside(std::vector<Jet> jets, std::vector<Electron *> els, std::vector<Muon *> mus, double dR){
...
bool HNtypeI_opt_ee::LeptonPassID(Lepton &lepton, TString ID){
  ...
  if(lepton.LeptonFlavour()==Lepton::ELECTRON){
    ...
    cerr << "[HNtypeI_opt_ee::LeptonPassID] lepton flavour wrong.." << endl;
    exit(EXIT_FAILURE);
    return false;
  }
...
void HNtypeI_opt_ee::FillWeightHist(TString label, double _weight){
  FillHist( "weights/"+ label , _weight ,1., 200, 0., 5,"ev weight");
}
void HNtypeI_opt_ee::FillCutFlow(bool IsCentral, TString suffix, TString histname, double weight){
...
double HNtypeI_opt_ee::LSFSF(int lepflav, int dir){
  ...
  if(lepflav==0){
    ...
    cerr << "[HNtypeI_opt_ee::LSFSF] wrong lepflav : " <<  lepflav << endl;
    exit(EXIT_FAILURE);
    return 1.;
  }
