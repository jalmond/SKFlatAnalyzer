#include "FakeRateHNMC.h"

void FakeRateHNMC::initializeAnalyzer(){
  ...
  cout << "[FakeRateHNMC::initializeAnalyzer()] RunSyst = " << RunSyst << endl;
  ...
  if(!IsDATA){
    ...
    cout << "[FakeRateHNMC::initializeAnalyzer()] PUfname = " << PUfname << endl;
    cout << "[FakeRateHNMC::initializeAnalyzer()] PUhname = " << PUhname << endl;
...
void FakeRateHNMC::executeEvent(){
  ...
  for(unsigned int it_id=0; it_id<ELIDs.size(); it_id++){
    ...
    param.Name = "FakeRateHNMC_"+ElectronTightID;
    ...
    //************************************************///
    param.Name = "FakeRateHNMC_"+channel[it_id] + "_" +Tight_ID;
...
void FakeRateHNMC::executeEventFromParameter(AnalyzerParameter param, TString El_ID, TString channel){
...
bool FakeRateHNMC::PassHNID(TString TightID, Electron el){
...
void FakeRateHNMC::RunM(std::vector<Electron> tight_el, std::vector<Electron> loose_el, std::vector<Muon> tight_mu, std::vector<Muon> loose_mu, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float w){
...
void FakeRateHNMC::RunE(std::vector<Electron> tight_el, std::vector<Electron> loose_el, std::vector<Muon> tight_mu, std::vector<Muon> loose_mu, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float w){
...
void FakeRateHNMC::GetFakeRateAndPromptRates(std::vector<Muon> muonLooseColl, TString mutag, std::vector<Muon> muonTightColl, float w, float isocut){
...
void FakeRateHNMC::GetFakeRateAndPromptRates(std::vector<Electron> electronLooseColl, TString eltag, std::vector<Electron> electronTightColl, float isocut,  float w, TString tightid ){
...
float FakeRateHNMC::GetPrescale( std::vector<Muon> muons,bool pass3, bool pass2, bool pass1 ){
...
double FakeRateHNMC::GetPrescale( std::vector<Electron> electrons,bool pass5,  bool pass4, bool pass3, bool pass2, bool pass1 ){
...
void FakeRateHNMC::MakeFakeRatePlots(TString label, TString mutag,   std::vector<Muon> muons_tight, std::vector<Muon> muons,  std::vector<Jet> jets, std::vector<Jet> alljets, float prescale_w, float w, float isocut, Particle MET){
...
void FakeRateHNMC::MakeFakeRatePlots(TString label, TString eltag,   std::vector<Electron> electrons_tight, std::vector<Electron> electrons,  std::vector<Jet> jets, std::vector<Jet> alljets, float prescale_w, float isocut ,float w, Particle MET){
...
bool FakeRateHNMC::UseEvent(std::vector<Muon> muons,  std::vector< Jet> jets, float awayjetcut, float precale_weight, Particle MET, float wt){
  bool useevent = false;
...
bool FakeRateHNMC::UseEvent(std::vector<Electron> electrons,  std::vector< Jet> jets, float awayjetcut, float precale_weight, Particle MET, float wt){
...
void FakeRateHNMC::GetFakeRates(std::vector<Muon> loose_mu, std::vector<Muon> tight_mu, TString tightlabel,  std::vector<Jet> jets,  std::vector<Jet> lljets, TString tag,double w, float isocut){
  ...
  if(!IsData)gens = GetGens();
  int type = GetLeptonType(loose_mu.at(0), gens);
  string str= to_string(type);  
  TString s_type = TString(str);
...
void FakeRateHNMC::GetFakeRates(std::vector<Electron> loose_el, std::vector<Electron> tight_el, TString tightlabel,  std::vector<Jet> jets,  std::vector<Jet> alljets, TString tag,float isocut, double w){
  

  if(!IsData)gens = GetGens();
  int type = GetLeptonType(loose_el.at(0), gens);
  string str= to_string(type);
  TString s_type = TString(str);
...
void FakeRateHNMC::FillRegionPlots( TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met,  double w){
...
double FakeRateHNMC::GetST( std::vector<Electron> electrons, std::vector<Muon> muons, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Particle met){
...
double FakeRateHNMC::MT(Particle a, Particle b){
...
std::vector<Jet> FakeRateHNMC::JetsVetoLeptonInside(std::vector<Jet> jets, std::vector<Electron *> els, std::vector<Muon *> mus, double dR){
...
bool FakeRateHNMC::LeptonPassID(Lepton &lepton, TString ID){
  ...
  if(lepton.LeptonFlavour()==Lepton::ELECTRON){
    ...
    cerr << "[FakeRateHNMC::LeptonPassID] lepton flavour wrong.." << endl;
    exit(EXIT_FAILURE);
    return false;
  }
...
void FakeRateHNMC::FillEventCutflow(int charge_i, int cf,float wt, TString cut,   TString label){
...
void FakeRateHNMC::FillWeightHist(TString label, double _weight){
  FillHist( "weights/"+ label , _weight ,1., 200, 0., 5,"ev weight");
}
void FakeRateHNMC::FillCutFlow(bool IsCentral, TString suffix, TString histname, double weight){
...
double FakeRateHNMC::LSFSF(int lepflav, int dir){
  ...
  if(lepflav==0){
    ...
    cerr << "[FakeRateHNMC::LSFSF] wrong lepflav : " <<  lepflav << endl;
    exit(EXIT_FAILURE);
    return 1.;
  }
