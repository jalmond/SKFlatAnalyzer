#include "SkimTree_ZZ.h"

void SkimTree_ZZ::initializeAnalyzer(){

  outfile->cd();
  cout << "[SkimTree_ZZ::initializeAnalyzer()] gDirectory = " << gDirectory->GetName() << endl;
  ...
  if(DataYear==2016){
    ...
    cout << "[SkimTree_ZZ::initializeAnalyzer] DataYear is wrong : " << DataYear << endl;
  }

  cout << "[SkimTree_ZZ::initializeAnalyzer] triggers to skim = " << endl;
  for(unsigned int i=0; i<triggers.size(); i++){
    cout << "[SkimTree_ZZ::initializeAnalyzer]   " << triggers.at(i) << endl;
  }

}

void SkimTree_ZZ::executeEvent(){
  ...
  if ( NLep >= 4 )  newtree->Fill();
  else if(allfatjets.size() > 0) newtree->Fill();
  else return;
...
void SkimTree_ZZ::executeEventFromParameter(AnalyzerParameter param){

}

SkimTree_ZZ::SkimTree_ZZ(){
