#include "HNtypeI_MCClosureDilepton.h"

void HNtypeI_MCClosureDilepton::initializeAnalyzer(){
  ...
  cout << "[HNtypeI_MCClosureDilepton::initializeAnalyzer()] RunFake = " << RunFake << endl;
  cout << "[HNtypeI_MCClosureDilepton::initializeAnalyzer()] RunCF = " << RunCF << endl;
  cout << "[HNtypeI_MCClosureDilepton::initializeAnalyzer()] RunSyst = " << RunSyst << endl;
  cout << "[HNtypeI_MCClosureDilepton::initializeAnalyzer()] IncludeFakeLepton = " << IncludeFakeLepton << endl;
...
void HNtypeI_MCClosureDilepton::executeEvent(){
  ...
  vector<HNtypeI_MCClosureDilepton::Flavour> channel;
  ...
  if(isEE||!IsDATA){
    ...
    ELIDs.push_back(make_pair("HNTightV1","HNVeto2016"));  el_loose_id.push_back("HNLooseV4");
  ...
  for(unsigned int it_id=0; it_id<ELIDs.size(); it_id++){
    ...
    param.Name = "HNtypeI_MCClosureDilepton_"+ElectronTightID;
    ...
    if(channel[it_id]==MuMu)param.Name = "HNtypeI_MCClosureDilepton_MuMu_" +Tight_ID;
    if(channel[it_id]==EE)  param.Name = "HNtypeI_MCClosureDilepton_EE_" +Tight_ID;
    //    if(channel[it_id]==EMu) param.Name = "HNtypeI_MCClosureDilepton_EMU_" +Tight_ID;
...
void HNtypeI_MCClosureDilepton::executeEventFromParameter(AnalyzerParameter param, TString Tight_ID, HNtypeI_MCClosureDilepton::Flavour channel){
  ...
  if(RunCF){
	...
	if(myelectrons.size() !=0){
	  if(!param.Electron_Tight_ID.Contains("HEEP"))this_electron.SetPtEtaPhiM( tmp_myelectrons.at(i).CalcPtCone(tmp_myelectrons.at(i).RelIso(), isocut_el), tmp_myelectrons.at(i).Eta(), tmp_myelectrons.at(i).Phi(), tmp_myelectrons.at(i).M() );
	}
	...
	//}
	if(ptcone_ok)mymuons.push_back( this_muon );
      }// muon loop
    }// MM channel
  }
  ...
  for(auto iel : myelectrons){
    if(IsData) myelectrons_noconv.push_back(iel);
    else{
      if(GetLeptonType(iel, gens) >= 0) myelectrons_noconv.push_back(iel);
    }
  }
  ...
  std::sort(myelectrons_noconv.begin(),myelectrons_noconv.end(), PtComparing);
  ...
  if(IsEE)    RunEE(-1,myelectrons, veto_electrons,mymuons,veto_muons,  jet_map, fatjet_map, ev,  param, weight);
  if(!IsEE)      RunMM(-1,myelectrons, veto_electrons,mymuons,veto_muons,  jet_map, fatjet_map, ev,  param, weight);

}



void HNtypeI_MCClosureDilepton::RunEECR(int ptbin, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float weight_ee ){
			    
  return;
}

void HNtypeI_MCClosureDilepton::RunMMCR(std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float weight_mm ){
...
void HNtypeI_MCClosureDilepton::RunMM(int ptbin, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::map<TString, std::vector<Jet> > map_jets, std::map<TString, std::vector<FatJet> > map_fatjets,  Event ev, AnalyzerParameter param,  float weight_mm ){
  

  HNtypeI_MCClosureDilepton::Flavour  flavour  = MuMu;
  ...
  HNtypeI_MCClosureDilepton::ChargeType charge_i = (muons[0].Charge() ==  muons[1].Charge() )  ?  SS : OS;
...
void HNtypeI_MCClosureDilepton::RunHighMassSR(HNtypeI_MCClosureDilepton::ChargeType charge_i, TString channel_s,HNtypeI_MCClosureDilepton::Flavour flavour, TString charge_s,Particle llCand, TString label, vector<Jet> jets, vector<FatJet> fatjets,  vector<Electron> electrons , vector<Muon> muons, Particle METv, int mPV, double weight_hm, Event ev, AnalyzerParameter param, bool FullAnalysis){
  ...
  if(charge_s=="opposite_sign" && inZpeak ) return;
...
void HNtypeI_MCClosureDilepton::FillAllSignalRegion1(HNtypeI_MCClosureDilepton::Flavour flavour, TString signal_region1, bool isdata, TString charge_s, TString label, std::vector<Jet> jets, std::vector<FatJet> fatjets, std::vector<Electron> electrons, std::vector<Muon> muons,   Particle _MET,int _npv  , double w    , bool FullAnalysis ){

  return;
}

void HNtypeI_MCClosureDilepton::FillAllSignalRegion2(HNtypeI_MCClosureDilepton::Flavour flavour, TString signal_region2, bool isdata, TString charge_s, TString label, std::vector<Jet> jets, std::vector<FatJet> fatjets, std::vector<Electron> electrons, std::vector<Muon> muons, Particle _MET,int _npv  , double w, bool FullAnalysis){
...
void  HNtypeI_MCClosureDilepton::FillObjectSizes(std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::map<TString, std::vector<Jet> > map_jets, std::map<TString, std::vector<FatJet> > map_fatjets,  Event ev, AnalyzerParameter param,  float weight_ll, TString  channel_s){

  TString label = param.Name;

  bool debug=false;
  if(debug){
    cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO " << endl;
    cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO  #electrons    = "<<  electrons.size() << endl;
    cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO  #muon         = "<<  muons.size() << endl;
    cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO  #electrons (veto) = "<<  electrons_veto.size() << endl;
    cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO  #muon      (veto) = "<<  muons_veto.size() << endl;
  }
  ...
  for(auto fatjet : map_fatjets){
    ...
    if(debug)cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO " << fatjet.first << " = " << fatjet.second.size() << endl;
  }
  for(auto jet : map_jets){
    FillHist( "NObj/n_ak4jet_"+jet.first+"_"+label+channel_s, jet.second.size() ,weight_ll, 10, 0., 10,"N_{AK4}");
    if(debug)cout << "HNtypeI_MCClosureDilepton::FillObjectSizes::INFO " << jet.first << " = " << jet.second.size() << endl;
  }
...
void HNtypeI_MCClosureDilepton::MakeAK4JetPlots(Flavour channel, AnalyzerParameter param,map<TString, vector<Jet> > jet_map, vector<Electron>  veto_electrons, vector<Muon>  veto_muons, double w){
...
void HNtypeI_MCClosureDilepton::MakeAK8JetPlots(Flavour channel, AnalyzerParameter param,map<TString, vector<FatJet> > fatjet_map, vector<Electron>  veto_electrons, vector<Muon>  veto_muons, double w){
...
vector<Jet>   HNtypeI_MCClosureDilepton::GetAK4Jets(vector<Jet> jets, double pt_cut ,  double eta_cut, bool lepton_cleaning  , double dr_lep_clean, double dr_ak8_clean, TString pu_tag, vector<Electron>  veto_electrons, vector<Muon>  veto_muons, vector<FatJet> fatjets){
...
vector<FatJet>   HNtypeI_MCClosureDilepton::GetAK8Jets(vector<FatJet> fatjets, double pt_cut ,  double eta_cut, bool lepton_cleaning  , double dr_lep_clean , bool apply_tau21, double tau21_cut , bool apply_masscut, double sdmass_lower_cut,  double sdmass_upper_cut, vector<Electron>  veto_electrons, vector<Muon>  veto_muons){
...
vector<pair<int, pair<TString, double> > >  HNtypeI_MCClosureDilepton::GetSyst(HNtypeI_MCClosureDilepton::Flavour flavour, bool isdata){
...
int HNtypeI_MCClosureDilepton::GetNBJets(TString ID, TString WP ){
...
void HNtypeI_MCClosureDilepton::RunEE(int ptbin, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::map<TString, std::vector<Jet> > map_jets, std::map<TString, std::vector<FatJet> > map_fatjets,  Event ev, AnalyzerParameter param,  float weight_ee){
  
  HNtypeI_MCClosureDilepton::Flavour flavour=EE;
  ...
  HNtypeI_MCClosureDilepton::ChargeType charge_i = (electrons[0].Charge() ==  electrons[1].Charge() )  ?  SS : OS;
...
double HNtypeI_MCClosureDilepton::GetEventFatJetSF(vector<FatJet> fatjets, TString label, int dir){
...
double HNtypeI_MCClosureDilepton::GetFatJetSF(FatJet fatjet, TString tag,  int dir){
...
double HNtypeI_MCClosureDilepton::GetMuonMCWeight(std::vector<Muon> muons, TString Tight_ID,AnalyzerParameter param ){
...
double HNtypeI_MCClosureDilepton::GetElectronMCWeight(std::vector<Electron> electrons, TString Tight_ID,AnalyzerParameter param ){
...
double HNtypeI_MCClosureDilepton::GetFakeRateEl(double eta, double pt, TString id){
...
double HNtypeI_MCClosureDilepton::GetFakeRateM(double eta, double pt, TString id){
...
double HNtypeI_MCClosureDilepton::CalculateDiLepWeight( double fr1, double fr2, bool mu1Tight, bool mu2Tight){
...
double HNtypeI_MCClosureDilepton::GetCFWeightElectron(std::vector<Electron> electrons , TString tight_id){
...
double HNtypeI_MCClosureDilepton:: GetFakeWeightElectron(std::vector<Electron> electrons , TString tight_id){
  ...
  if(electrons.size() == 2){
    ...
    TString postfix = "";
    double fr1 = GetFakeRateEl(_el1_eta, electrons.at(0).Pt(), histtag+postfix);
...
double HNtypeI_MCClosureDilepton:: GetFakeWeightMuon(std::vector<Muon> muons , TString tight_id){
...
HNtypeI_MCClosureDilepton::HNtypeI_MCClosureDilepton(){
...
void HNtypeI_MCClosureDilepton::FilAllMuonPlots(TString label , TString cut,  std::vector<Muon> muons, float w){
...
void HNtypeI_MCClosureDilepton::FilAllElectronPlots(TString label , TString cut,  std::vector<Electron> els, float w){
...
void HNtypeI_MCClosureDilepton::MakeGeneralPlots(vector<Muon> mymuons, vector<Electron> myelectrons, map<TString, vector<FatJet> > fatjet_map, AnalyzerParameter param ,double weight){
...
void HNtypeI_MCClosureDilepton::FillZZCRPlots(HNtypeI_MCClosureDilepton::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
  ...
  double lep2_ptcut= (flavour==EE) ?   15. : 10.;
  ...
  if(flavour == MuMu){
    if(muons.size() ==4 && electrons_veto.size() == 0 && muons_veto.size()==4){
      if(muons[0].Pt() > lep1_ptcut && muons[1].Pt()  > lep2_ptcut && muons[3].Pt()  > 10) {
	if(NBJets ==0){
...
double HNtypeI_MCClosureDilepton::GetST( std::vector<Electron> electrons, std::vector<Muon> muons, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Particle met){
...
double HNtypeI_MCClosureDilepton::GetST( std::vector<Electron> electrons, std::vector<Muon> muons, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev){
...
void HNtypeI_MCClosureDilepton::FillZGCRPlots(HNtypeI_MCClosureDilepton::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
  ...
  double lep1_ptcut= (flavour==EE) ?   25. : 20.;
  double lep2_ptcut= (flavour==EE) ?   15. : 10.;
  double lep3_ptcut= (flavour==EE) ?   15. : 10.;
...
void HNtypeI_MCClosureDilepton::FillWGCRPlots(HNtypeI_MCClosureDilepton::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
...
void HNtypeI_MCClosureDilepton::FillWZCRPlots(HNtypeI_MCClosureDilepton::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
...
void HNtypeI_MCClosureDilepton::MakeSignalPlots(){
  bool mupt_pass=true;
  int mother_nu(0);
...
double  HNtypeI_MCClosureDilepton::GetMass(TString type , std::vector<Jet> jets, std::vector<FatJet> fatjets){
...
void HNtypeI_MCClosureDilepton::FillSigRegionPlots1(HNtypeI_MCClosureDilepton::Flavour channel, int systdir, TString syst, double sysval, TString label_1, TString label_2, TString label_3,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7, double var8, double var9, double var10, double var11){
  ...
  if(!label_1.Contains("highmass")){
    ...
    HNtypeI_MCClosureDilepton::Region cutflow_index = MuMu_SR1;
    if(channel == EE )cutflow_index = EE_SR1;
    HNtypeI_MCClosureDilepton::ChargeType charge_i = (lep1.Charge() ==  lep2.Charge() )  ?  SS : OS;
...
void HNtypeI_MCClosureDilepton::FillSigRegionPlots2(HNtypeI_MCClosureDilepton::Flavour channel, int systdir, TString syst, double sysval,TString label_1, TString label_2, TString label_3,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7){
  ...
  if(!label_1.Contains("highmass")){
    ...
    HNtypeI_MCClosureDilepton::Region  cutflow_index = MuMu_SR2;
...
void HNtypeI_MCClosureDilepton::FillRegionPlots(HNtypeI_MCClosureDilepton::Flavour channel, bool plotCR, TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w){
...
double HNtypeI_MCClosureDilepton::MT(Particle a, Particle b){
...
std::vector<Jet> HNtypeI_MCClosureDilepton::JetsVetoLeptonInside(std::vector<Jet> jets, std::vector<Electron *> els, std::vector<Muon *> mus, double dR){
...
void HNtypeI_MCClosureDilepton::FillEventCutflow(HNtypeI_MCClosureDilepton::ChargeType charge_i, HNtypeI_MCClosureDilepton::Flavour cf_flavour,float wt, TString cut,   TString label){
  ...
  if(cf_flavour == EE){
    ...
    vector<TString> labels_os = {"NoCut", "METFilter", "Trigger_el", 
				 ...
				 "Highmass_BJT_SR3",
				 "Highmass_BJT_SR4"};
    if(charge_i == SS)    AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_diel_"+charge_, wt, labels_ss, cut);
    else if(charge_i == OS)    AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_diel_"+charge_, wt, labels_os, cut);
    else {
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_diel_same_sign", wt, labels_ss, cut);
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_diel_opposite_sign", wt, labels_os, cut);
    }
    return;
  }
  ...
  if(cf_flavour == MuMu){
    ...
    if(charge_i == SS)    AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_dimu_"+charge_, wt, labels_ss, cut);
    else if(charge_i == OS)    AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_dimu_"+charge_, wt, labels_os, cut);
    else {
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_dimu_same_sign", wt, labels_ss, cut);
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_dimu_opposite_sign", wt, labels_os, cut);
    }
    return;
  }
...
void HNtypeI_MCClosureDilepton::FillEventCutflow(HNtypeI_MCClosureDilepton::ChargeType charge_i,Region cf,float wt, TString cut,   TString label){                                                            
  ...
  if(cf == MuMu_SR1){
    ...
    if(charge_i  == SS ) {
      ...
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_MuMu_SR1", wt, labels, cut);
    }
    else {
      for (unsigned int x=0; x < masses.size(); x++) labels.push_back("SR3_mn"+masses[x] );
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_MuMu_SR3", wt, labels, cut);
  ...
  if(cf== EE_SR1){
    ...
    if(charge_i  == SS ) {
      for (unsigned int x=0; x < masses.size(); x++) labels.push_back("SR1_mn"+masses[x] );
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_EE_SR1", wt, labels, cut);
    }
    else{
      for (unsigned int x=0; x < masses.size(); x++) labels.push_back("SR3_mn"+masses[x] );

      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_EE_SR3", wt, labels, cut);
  ...
  if(cf== MuMu_SR2){
    ...
    if(charge_i  == SS ) {
      ...
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_MuMu_SR2", wt, labels, cut);
    }
    else{
      for (unsigned int x=0; x < masses.size(); x++) labels.push_back("SR4_mn"+masses[x] );
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_MuMu_SR4", wt, labels, cut);
  ...
  if(cf== EE_SR2){
    ...
    if(charge_i  == SS ) {
      ...
      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_EE_SR2", wt, labels, cut);
    }
    else{
      for (unsigned int x=0; x < masses.size(); x++) labels.push_back("SR4_mn"+masses[x] );

      AnalyzerCore::FillEventCutflowAll(label+"exo_17_028_EE_SR4", wt, labels, cut);
...
void HNtypeI_MCClosureDilepton::FillWeightHist(TString label, double _weight){
  FillHist( "weights/"+ label , _weight ,1., 200, -5., 5,"ev weight");
}
void HNtypeI_MCClosureDilepton::FillCutFlow(bool IsCentral, TString suffix, TString histname, double weight){
