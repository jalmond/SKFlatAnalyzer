...
void HNtypeI_JA::initializeAnalyzer(){
  ...
  cout << "[HNtypeI_JA::initializeAnalyzer()] IncludeFakeLepton = " << IncludeFakeLepton << endl;
...
void HNtypeI_JA::executeEvent(){
  ...
  vector<HNtypeI_JA::Flavour> channel;

  if(isEE||!IsDATA){

    // POG T + IP + tight charge
    ...
    // TIGHT POG + no tight_charge in low pt                                                                                      

    // nocc == no tight charge                                                                                                    
  ...
  if(isMM||!IsDATA){
    // POGTightPFIsoVeryTight : IP < 3. | iso < 0.15 | POGTight 
  ...
  for(unsigned int it_id=0; it_id<ELIDs.size(); it_id++){
    ...
    if(channel[it_id]==MuMu)param.Name = "HNtypeI_JA_MuMu_" +Tight_ID;
    if(channel[it_id]==EE)  param.Name = "HNtypeI_JA_EE_" +Tight_ID;
    //    if(channel[it_id]==EMu) param.Name = "HNtypeI_JA_EMU_" +Tight_ID;
...
void HNtypeI_JA::executeEventFromParameter(AnalyzerParameter param, TString Tight_ID, HNtypeI_JA::Flavour channel){
  ...
  std::vector<Muon>      tmp_mymuons2     = SelectMuons    (this_AllMuons,    "POGHighPtMixTight", ptmuon, 2.4);
  ...
  FilAllElectronPlots(param.Electron_Veto_ID , "el",  veto_electrons,1.);

  if(weight < 0) weight = 0.;
  for(auto i : veto_muons){
    //cout << "Type " <<  GetLeptonType(i, gens) << endl;
    
    FillHist( "muon_relsio_type_"+param.Muon_Tight_ID, i.RelIso() ,GetLeptonType(i, gens) , weight, 1000, 0., 10, 20, -10,10.);
    FillHist( "muon_dxy_type_"+param.Muon_Tight_ID, i.dXY() ,GetLeptonType(i, gens) , weight, 1000, -0.5, 0.5, 20, -10,10.);
    FillHist( "muon_ip3d_type_"+param.Muon_Tight_ID, i.IP3D()/i.IP3Derr() ,GetLeptonType(i, gens) , weight, 1000, -20., 20, 20, -10,10.);

    std::string sd = std::to_string(double(GetLeptonType(i, gens)));
    FillHist( "muon_relsio_type_"+TString(sd)+param.Muon_Tight_ID, i.RelIso() ,weight, 1000, 0., 2);
    FillHist( "muon_dxy_type_"+TString(sd)+param.Muon_Tight_ID, i.dXY() , weight, 1000, -0.5, 0.5) ;
    FillHist( "muon_ip3d_type_"+TString(sd)+param.Muon_Tight_ID, i.IP3D()/i.IP3Derr() ,weight, 1000, -20., 20);

  }
  for(auto i : veto_electrons){
    std::string sd = std::to_string(double(GetLeptonType(i, gens)));

    FillHist( "electron_relsio_type_"+param.Electron_Tight_ID, i.RelIso() ,GetLeptonType(i, gens) , weight, 1000, 0., 10, 20, -10,10.);
    FillHist( "electron_dxy_type_"+param.Electron_Tight_ID, i.dXY() ,GetLeptonType(i, gens) , weight, 1000, -0.5, 0.5, 20, -10,10.);
    FillHist( "electron_ip3d_type_"+param.Electron_Tight_ID, i.IP3D()/i.IP3Derr() ,GetLeptonType(i, gens) , weight, 1000, -20., 20, 20, -10,10.);

    FillHist( "electron_relsio_type_"+TString(sd)+param.Electron_Tight_ID, i.RelIso() , weight, 1000, 0., 10.);
    FillHist( "electron_dxy_type_"+TString(sd)+param.Electron_Tight_ID, i.dXY()  , weight, 1000, -0.5, 0.5);
    FillHist( "electron_ip3d_type_"+TString(sd)+param.Electron_Tight_ID, i.IP3D()/i.IP3Derr() , weight, 1000, -20., 20);
    
  }

  if(SameCharge(tmp_mymuons)){
    FillHist( "ss_tightmuon_type_"+param.Muon_Tight_ID, GetLeptonType(tmp_mymuons[0], gens) ,GetLeptonType(tmp_mymuons[1], gens) , weight, 20, -10., 10, 20, -10,10.);
    if(tmp_mymuons[1].Pt() > 15 )FillHist( "ss_tpt_tightmuon_type_"+param.Muon_Tight_ID, GetLeptonType(tmp_mymuons[0], gens) ,GetLeptonType(tmp_mymuons[1], gens) , weight, 20, -10., 10, 20, -10,10.);
  }
  
  if(SameCharge(tmp_myelectrons)){
    FillHist( "ss_tightelectron_type_"+param.Electron_Tight_ID, GetLeptonType(tmp_myelectrons[0], gens) ,GetLeptonType(tmp_myelectrons[1], gens) , weight, 20, -10., 10, 20, -10,10.);
    if(tmp_myelectrons[1].Pt() > 20.) FillHist( "ss_tpt_tightelectron_type_"+param.Electron_Tight_ID, GetLeptonType(tmp_myelectrons[0], gens) ,GetLeptonType(tmp_myelectrons[1], gens) , weight, 20, -10., 10, 20, -10,10.);
  }
  if(tmp_mymuons.size() ==3){
    if(tmp_mymuons[2].Pt() > 15 )    FillHist( "lll_tightmuon_type_"+param.Muon_Tight_ID, GetLeptonType(tmp_mymuons[0], gens) ,GetLeptonType(tmp_mymuons[1], gens) , weight, 20, -10., 10, 20, -10,10.);
  }
  
  if(tmp_myelectrons.size()==3){
    if(tmp_myelectrons[2].Pt() > 20.)FillHist( "lll_tpt_tightelectron_type_"+param.Electron_Tight_ID, GetLeptonType(tmp_myelectrons[0], gens) ,GetLeptonType(tmp_myelectrons[1], gens) , weight, 20, -10., 10, 20, -10,10.);
  }
  ...
  if(IsEE)RunEE(0,myelectrons, veto_electrons,mymuons,veto_muons,  jets, fatjets, ev,  param, weight);
  else RunMM(0,myelectrons, veto_electrons,mymuons,veto_muons,  jets, fatjets, ev,  param, weight);

}



void HNtypeI_JA::RunEECR(std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float weight_ee ){
  ...
  if(!IsDATA) weight_ee*= GetElectronMCWeight(electrons, param.Electron_Tight_ID, param);
...
void HNtypeI_JA::RunMMCR(std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float weight_mm ){
  ...
  if(!IsDATA) weight_mm*= GetMuonMCWeight(muons, param.Muon_Tight_ID, param);

  TString label    = param.Name;
  TString channel_s="MuMu";
  HNtypeI_JA::Flavour  flavour  = MuMu;
...
void HNtypeI_JA::RunMM(int ptbin, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float weight_mm ){
  ...
  int  NBJets_medium      =  GetNBJets("tight","Medium");
  if(llCand.M() > 81. && llCand.M() < 101)     FillRegionPlots(flavour,true,"Z_MuMu_" , label, jets,  fatjets,  electrons, muons,  METv, nPV, weight_mm);
							       
  


  if(NBJets_medium > 1&& jets.size() > 3) FillRegionPlots(flavour,true,"TT_MuMu_" , label, jets,  fatjets,  electrons, muons,  METv,nPV, weight_mm);


  return;
}

void HNtypeI_JA::RunHighMassSR(HNtypeI_JA::ChargeType charge_i, TString channel_s,HNtypeI_JA::Flavour flavour, TString charge_s,Particle llCand, TString label, vector<Jet> jets, vector<FatJet> fatjets,  vector<Electron> electrons , vector<Muon> muons, Particle METv, int mPV, double weight_hm, Event ev, AnalyzerParameter param, bool FullAnalysis){
...
void HNtypeI_JA::FillAllSignalRegion1(HNtypeI_JA::Flavour flavour, TString signal_region1, bool isdata, TString charge_s, TString label, std::vector<Jet> jets, std::vector<FatJet> fatjets, std::vector<Electron> electrons, std::vector<Muon> muons,   Particle _MET,int _npv  , double w    , bool FullAnalysis ){
...
void HNtypeI_JA::FillAllSignalRegion2(HNtypeI_JA::Flavour flavour, TString signal_region2, bool isdata, TString charge_s, TString label, std::vector<Jet> jets, std::vector<FatJet> fatjets, std::vector<Electron> electrons, std::vector<Muon> muons, Particle _MET,int _npv  , double w, bool FullAnalysis){
...
void  HNtypeI_JA::FillObjectSizes(std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::map<TString, std::vector<Jet> > map_jets, std::map<TString, std::vector<FatJet> > map_fatjets,  Event ev, AnalyzerParameter param,  float weight_ll, TString  channel_s){

  TString label = param.Name;

  bool debug=false;
  if(debug){
    cout << "HNtypeI_JA::FillObjectSizes::INFO " << endl;
    cout << "HNtypeI_JA::FillObjectSizes::INFO  #electrons    = "<<  electrons.size() << endl;
    cout << "HNtypeI_JA::FillObjectSizes::INFO  #muon         = "<<  muons.size() << endl;
    cout << "HNtypeI_JA::FillObjectSizes::INFO  #electrons (veto) = "<<  electrons_veto.size() << endl;
    cout << "HNtypeI_JA::FillObjectSizes::INFO  #muon      (veto) = "<<  muons_veto.size() << endl;
  }
  ...
  for(auto fatjet : map_fatjets){
    ...
    if(debug)cout << "HNtypeI_JA::FillObjectSizes::INFO " << fatjet.first << " = " << fatjet.second.size() << endl;
  }
  for(auto jet : map_jets){
    FillHist( "NObj/n_ak4jet_"+jet.first+"_"+label+channel_s, jet.second.size() ,weight_ll, 10, 0., 10,"N_{AK4}");
    if(debug)cout << "HNtypeI_JA::FillObjectSizes::INFO " << jet.first << " = " << jet.second.size() << endl;
  }
...
void HNtypeI_JA::MakeAK4JetPlots(Flavour channel, AnalyzerParameter param,map<TString, vector<Jet> > jet_map, vector<Electron>  veto_electrons, vector<Muon>  veto_muons, double w){
...
void HNtypeI_JA::MakeAK8JetPlots(Flavour channel, AnalyzerParameter param,map<TString, vector<FatJet> > fatjet_map, vector<Electron>  veto_electrons, vector<Muon>  veto_muons, double w){
...
vector<Jet>   HNtypeI_JA::GetAK4Jets(vector<Jet> jets, double pt_cut ,  double eta_cut, bool lepton_cleaning  , double dr_lep_clean, double dr_ak8_clean, TString pu_tag, vector<Electron>  veto_electrons, vector<Muon>  veto_muons, vector<FatJet> fatjets){
...
vector<FatJet>   HNtypeI_JA::GetAK8Jets(vector<FatJet> fatjets, double pt_cut ,  double eta_cut, bool lepton_cleaning  , double dr_lep_clean , bool apply_tau21, double tau21_cut , bool apply_masscut, double sdmass_lower_cut,  double sdmass_upper_cut, vector<Electron>  veto_electrons, vector<Muon>  veto_muons){
...
vector<pair<int, pair<TString, double> > >  HNtypeI_JA::GetSyst(HNtypeI_JA::Flavour flavour, bool isdata){
...
int HNtypeI_JA::GetNBJets(TString ID, TString WP ){
...
void HNtypeI_JA::RunEE(int ptbin, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, AnalyzerParameter param,  float weight_ee){
  
  HNtypeI_JA::Flavour flavour=EE;
  ...
  if(electrons[1].Pt() < 20.) return;
  if(!IsDATA) weight_ee*= GetElectronMCWeight(electrons, param.Electron_Tight_ID, param);
  if(SameCharge(electrons)) return;
  Particle llCand = electrons.at(0) + electrons.at(1);
  if(llCand.M() > 81. && llCand.M() < 101)     FillRegionPlots(flavour,true,"Z_EE_" , label, jets,  fatjets,  electrons, muons,  METv, nPV, weight_ee);
						    
  int  NBJets_medium      =  GetNBJets("tight","Medium");

  if(NBJets_medium > 1&& jets.size() > 3) FillRegionPlots(flavour,true,"TT_EE_" , label, jets,  fatjets,  electrons, muons,  METv,nPV, weight_ee);
...
double HNtypeI_JA::GetEventFatJetSF(vector<FatJet> fatjets, TString label, int dir){
...
double HNtypeI_JA::GetFatJetSF(FatJet fatjet, TString tag,  int dir){
...
double HNtypeI_JA::GetMuonMCWeight(std::vector<Muon> muons, TString Tight_ID,AnalyzerParameter param ){
  ...
  if(!IsDATA){
    ...
    */
	...
	this_isosf = mcCorr->MuonISO_SF("NUM_LooseRelTkIso_DEN_HighPtIDandIPCut", mu.Eta(), mu.MiniAODPt(),  0);
      ...
      if(Muon_ID_SF_Key == "POGHighPtMixTight"){
	bool usehighpt(false);
	if(muons.size()==2){
	  if(muons[1].Pt() > 60){usehighpt=true;}
	}
	if(usehighpt){
	  this_recosf = mcCorr->MuonReco_SF("HighPtMuonRecoSF", mu.Eta(), MiniAODP, 0);
	  this_idsf  = mcCorr->MuonID_SF ("NUM_HighPtID_DEN_genTracks" ,  mu.Eta(), mu.MiniAODPt(),  0);
...
double HNtypeI_JA::GetElectronMCWeight(std::vector<Electron> electrons, TString Tight_ID,AnalyzerParameter param ){
...
double HNtypeI_JA::GetFakeRateEl(double eta, double pt, TString id){
...
double HNtypeI_JA::GetFakeRateM(double eta, double pt, TString id){
...
double HNtypeI_JA::CalculateDiLepWeight( double fr1, double fr2, bool mu1Tight, bool mu2Tight){
...
double HNtypeI_JA::GetCFWeightElectron(std::vector<Electron> electrons , TString tight_id){
...
double HNtypeI_JA:: GetFakeWeightElectron(std::vector<Electron> electrons , TString tight_id){
...
double HNtypeI_JA:: GetFakeWeightMuon(std::vector<Muon> muons , TString tight_id){
  ...
  if(muons.size() == 2) {
    ...
    if(tight_id == "POGHighPtMixTight") {
      ...
      if(muons[0].Pt() > 60 && muons[1].Pt() > 60){
	histtag1 = "POGHighPtTight";
	histtag2 = "POGHighPtTight";
      }
      else{
	histtag1 = "POGTightPFIsoVeryVeryTight";
	histtag2 = "POGTightPFIsoVeryVeryTight";
      }
    }
  ...
  TString histtag=tight_id;
  if(tight_id == "POGHighPtMixTight" && lowpt) histtag="POGTightPFIsoVeryVeryTight";
  if(tight_id == "POGHighPtMixTight" && !lowpt) histtag="POGHighPtTight";
...
void HNtypeI_JA::FilAllElectronPlots(TString label , TString cut,  std::vector<Electron> els, float w){
  ...
  for(unsigned int i=0; i < els.size(); i++){
    ...
    int type1=fabs(GetLeptonType(els[i], gens));
    if(IsCF(els[i],gens))     FillHist( cut+ "/dxy_cf__"+el_lab+label , els.at(i).dXY() , w, 500, -0.2, 0.2, "dXY");
    else   if(type1 == 5)  FillHist( cut+ "/dxy_type5__"+el_lab+label , els.at(i).dXY() , w, 500, -0.2, 0.2, "dXY");
    else   if(type1 == 6)  FillHist( cut+ "/dxy_type6__"+el_lab+label , els.at(i).dXY() , w, 500, -0.2, 0.2, "dXY");
    else   if(type1 == -5)  FillHist( cut+ "/dxy_cf_typem5_"+el_lab+label , els.at(i).dXY() , w, 500, -0.2, 0.2, "dXY");
    else   if(type1 == -6)  FillHist( cut+ "/dxy_cf_typem6_"+el_lab+label , els.at(i).dXY() , w, 500, -0.2, 0.2, "dXY");
    else if(type1 >= 0)   FillHist( cut+ "/dxy_cf_type0_"+el_lab+label , els.at(i).dXY() , w, 500, -0.2, 0.2, "dXY");
    
    if(IsCF(els[i],gens)) FillHist( cut+ "/IP3D_cf_"+el_lab+ label  , els.at(i).IP3D()/els.at(i).IP3Derr(), w, 400, -20., 20., "IP3D");
    else   if(type1 == 5) FillHist( cut+ "/IP3D_type5_"+el_lab+ label  , els.at(i).IP3D()/els.at(i).IP3Derr(), w, 400, -20., 20., "IP3D");
    else   if(type1 == 6)  FillHist( cut+ "/IP3D_type6_"+el_lab+ label  , els.at(i).IP3D()/els.at(i).IP3Derr(), w, 400, -20., 20., "IP3D");
    else   if(type1 == -5) FillHist( cut+ "/IP3D_typem5_"+el_lab+ label  , els.at(i).IP3D()/els.at(i).IP3Derr(), w, 400, -20., 20., "IP3D");
    else   if(type1 == -6) FillHist( cut+ "/IP3D_typem6_"+el_lab+ label  , els.at(i).IP3D()/els.at(i).IP3Derr(), w, 400, -20., 20., "IP3D");
    else if(type1 >= 0)  FillHist( cut+ "/IP3D_type0_"+el_lab+ label  , els.at(i).IP3D()/els.at(i).IP3Derr(), w, 400, -20., 20., "IP3D");
    continue;
...
void HNtypeI_JA::MakeGeneralPlots(vector<Muon> mymuons, vector<Electron> myelectrons, map<TString, vector<FatJet> > fatjet_map, AnalyzerParameter param ,double weight){
...
void HNtypeI_JA::FillZZCRPlots(HNtypeI_JA::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
...
void HNtypeI_JA::FillZGCRPlots(HNtypeI_JA::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){

  
  return;

}
void HNtypeI_JA::FillWGCRPlots(HNtypeI_JA::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
...
void HNtypeI_JA::FillWZCRPlots(HNtypeI_JA::Flavour flavour, std::vector<Electron> electrons, std::vector<Electron> electrons_veto, std::vector<Muon> muons, std::vector<Muon> muons_veto, std::vector<Jet> jets, std::vector<FatJet> fatjets,  Event ev, TString label, float w){
...
void HNtypeI_JA::FillSigRegionPlots1(HNtypeI_JA::Flavour channel, int systdir, TString syst, double sysval, TString label_1, TString label_2, TString label_3,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7, double var8, double var9, double var10, double var11){
  ...
  if(!label_1.Contains("highmass")){
    ...
    HNtypeI_JA::Region cutflow_index = MuMu_SR1;
    if(channel == EE )cutflow_index = EE_SR1;
    HNtypeI_JA::ChargeType charge_i = (lep1.Charge() ==  lep2.Charge() )  ?  SS : OS;
...
void HNtypeI_JA::FillSigRegionPlots2(HNtypeI_JA::Flavour channel, int systdir, TString syst, double sysval,TString label_1, TString label_2, TString label_3,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w,  double var1,  double var2, double var3, double var4, double var5, double var6, double var7){
  ...
  if(!label_1.Contains("highmass")){
    ...
    HNtypeI_JA::Region  cutflow_index = MuMu_SR2;
...
void HNtypeI_JA::FillRegionPlots(HNtypeI_JA::Flavour channel, bool plotCR, TString label_1, TString label_2,  std::vector<Jet> jets, std::vector<FatJet> fatjets,  std::vector<Electron> els, std::vector<Muon> mus, Particle  met, double nvtx,  double w){
  ...
  double ptbins[3] = {  10.,40.,2000.};
  double etabins[3] = { 0., 1.479,  2.5};
  double lep1_pt=lep1.Pt();
  double lep2_pt=lep2.Pt();
  double lep1_eta = fabs(lep1.Eta());
  double lep2_eta = fabs(lep2.Eta());
  int bin1(-1), bin2(-1);
  if(lep1_pt > 10. && lep1_pt < 40.) {
    if(lep1_eta < 1.479) bin1=1;
    else     if(lep1_eta < 2.4) bin1=2;
  }
  else {
    if(lep1_eta < 1.479) bin1=3;
    else     if(lep1_eta < 2.4) bin1=4;
  }
  if(lep2_pt > 10. && lep2_pt <40.) {
    if(lep2_eta < 1.479) bin2=1;
    else     if(lep2_eta < 2.4) bin2=2;
  }
  else {
    if(lep2_eta < 1.479) bin2=3;
    else     if(lep2_eta < 2.4) bin2=4;
  }

  if(bin1==bin2){
    FillHist( label_1+ "/"+ label_1 +  "_lep_pt_eta_" + label_2, lep1.Pt(), fabs(lep1.Eta())  , w, 2, ptbins, 2 , etabins);
    FillHist( label_1+ "/"+ label_1 +  "_lep_pt_eta_" + label_2, lep2.Pt(), fabs(lep2.Eta())  , w, 2, ptbins, 2 , etabins);
  }
  ...
  if(nvtx < 5) _nvtx=5;
  if(nvtx > 59) _nvtx=59;
  FillHist( label_1+ "/"+ label_1 + "_nPV_"+ label_2,  _nvtx , w, 60, 0., 60.);
...
void HNtypeI_JA::FillEventCutflow(HNtypeI_JA::ChargeType charge_i, HNtypeI_JA::Flavour cf_flavour,float wt, TString cut,   TString label){
...
void HNtypeI_JA::FillEventCutflow(HNtypeI_JA::ChargeType charge_i,Region cf,float wt, TString cut,   TString label){                                                            
