...
void DrawLimits(TString year="", TString channel="", bool CompareLimits=true){
  ...
  vector<TString> myWPs = {"240422_HNL_ULID"};
  vector<TString> tags = {"_sronly_syst","_sronly_syst_Run2Scaled"};
  //vector<TString> tags = {"_sronly_syst"};
  for(int i=0; i<myWPs.size(); i++){
    for(int j=0; j<tags.size(); j++){
      TString myWP = myWPs.at(i);
      TString tag = tags.at(j);
      ...
      files.push_back(filepath+myWP+"/"+year+"_"+channel+tag+"_"+method+"_limit.txt"); // This will be the nominal
      ...
      for(int i=0; i<files.size(); i++){
        ...
        int n_central = 25; //FIXME depending on the bins
        ...
        while(getline(in,elline)){
          ...
          ////==== skip points below 0
          //if(this_obs<=0 || this_limit<=0 || this_onesig_left<=0 || this_onesig_right<=0 || this_twosig_left<=0 || this_twosig_right<=0){
          //  n_central--;
          //  continue;
          //}
      ...
      //gr_exp_2->SetLineColor(kGreen+2);
...
//    =======================================
      ...
      if(CompareLimits){
        ...
        dummy2->GetYaxis()->SetRangeUser(0.85, 50);
        dummy2->SetTitle("");
        dummy2->Draw("hist");
        ...
        c_Dilep->SaveAs(this_plotpath+"/"+year+"_"+channel+"_13TeV_mixing_"+channel+"_"+myWP+tag+"_comp.png");
      }
      else c_Dilep->SaveAs(this_plotpath+"/"+year+"_"+channel+"_13TeV_mixing_"+channel+"_"+myWP+tag+".png");
    }
  }
